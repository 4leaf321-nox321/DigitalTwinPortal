        ...
        try {
          ...
          if (data.version && data.data && data.settings) {
            ...
            if (!data.data.nodes || !Array.isArray(data.data.nodes)) {
              if (showError) {
                showError('잘못된 JSON 파일입니다. data.nodes 배열이 필요합니다.');
              }
              return;
            }
            ...
            if (!data.data.edges || !Array.isArray(data.data.edges)) {
              if (showError) {
                showError('잘못된 JSON 파일입니다. data.edges 배열이 필요합니다.');
              }
              return;
            }
          }
          // 기존 형식 감지
            ...
            if (!Array.isArray(data.nodes)) {
              if (showError) {
                showError('잘못된 JSON 파일입니다. nodes 배열이 필요합니다.');
              }
              return;
            }
            ...
            if (!Array.isArray(data.edges)) {
              if (showError) {
                showError('잘못된 JSON 파일입니다. edges 배열이 필요합니다.');
              }
              return;
            }
          }
            ...
            if (showError) {
              showError('잘못된 JSON 파일입니다. 올바른 데이터 구조가 필요합니다.');
            }
            return;
          }
          ...
          if (showError) {
            showError('잘못된 JSON 파일입니다. 파일 형식을 확인해주세요.');
          }
        }
      };
