...
import { assignDistinctColors } from '../../utils/colorUtils';
...
const BulkEdgeModal = ({ 
  ...
  const [activeField, setActiveField] = useState(null); // 'from' 또는 'to'
  ...
  const getNodeIdByLabel = (label) => {
    ...
    return node ? node.id : label; // 라벨로 찾지 못하면 원래 값 반환
  };

  // 노드 라벨을 ID로 변환하는 함수
  const getLabelByNodeId = (id) => {
    const node = graphData.nodes.find(n => n.id === id);
    return node ? node.label : id; // ID로 찾지 못하면 원래 값 반환
  };
  ...
  const parseClipboardData = (text) => {
    ...
    lines.forEach(line => {
      ...
      if (cleanedColumns.length >= 2) {
        ...
        if (fromNode && toNode) { // 시작과 끝 노드가 있는 경우만 추가
          parsedData.push({
            from: fromNode,
            to: toNode,
            type: edgeType,
...
3. 페이지 새로고침 후 다시 시도`, '클립보드 접근 오류');
  ...
  const addSampleData = async () => {
    const availableNodes = graphData.nodes.slice(0, 5); // 처음 5개 노드만 사용
