...
export const setupNetworkEventListeners = ({
    ...
    if (params.nodes.length > 0) {
      ...
      if (layoutType === 'force-directed') {
        networkRef.current.setOptions({
          physics: {
            enabled: false // 드래그 중에는 물리 시뮬레이션 완전 비활성화
          }
        });
      }
      ...
      const draggedNodeUpdates = params.nodes.map(nodeId => ({
        id: nodeId,
        physics: false,
        fixed: { x: false, y: false } // 드래그는 가능하지만 고정은 아님
      }));
  ...
  networkRef.current.on('dragging', (params) => {
    // 드래그 중에는 추가적인 처리 없음 - vis-network가 자동으로 처리
  });

  networkRef.current.on('dragEnd', (params) => {
    if (params.nodes.length > 0) {
      // 드래그 완료 후 노드 위치 영구 저장
      const draggedNodes = params.nodes;
      
      // 최종 위치를 가져와서 노드 데이터에 영구 저장
      draggedNodes.forEach(nodeId => {
        try {
          ...
          if (finalPosition && !isNaN(finalPosition.x) && !isNaN(finalPosition.y) && nodesRef.current) {
            // 위치를 영구 저장하고 물리 시뮬레이션 비활성화
            nodesRef.current.update({
              id: nodeId,
              x: finalPosition.x,
              y: finalPosition.y,
              physics: false, // 위치 고정 - 물리 시뮬레이션의 영향을 받지 않음
              fixed: { x: false, y: false } // 향후 드래그는 가능
            });
          }
        } catch (error) {
      ...
      // force-directed 레이아웃에서는 잠시 후 물리 시뮬레이션 재활성화
      // 하지만 드래그된 노드들은 physics: false이므로 영향받지 않음
      if (layoutType === 'force-directed') {
        setTimeout(() => {
          if (networkRef.current) {
            try {
              ...
              // 짧은 시간 후 다시 비활성화 (다른 노드들의 미세 조정을 위해)
              setTimeout(() => {
                if (networkRef.current) {
                  networkRef.current.setOptions({
