...
export default async function run() {
  ...
  try {
    ...
    const nodesOut = [...document.querySelectorAll('[data-fg-node]')].map(n => n.textContent);
    say(nodesOut.length === 3 && nodesOut.includes('Teamcenter') && nodesOut.includes('메일') && nodesOut.includes('원가 산정 시스템'), `⑥ 시스템 노드 셋: ${nodesOut}`);
