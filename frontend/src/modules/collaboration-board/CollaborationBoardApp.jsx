...
import {
  ...
  ChevronDown, ChevronUp, ArrowLeft, ClipboardList, BarChart3,
  CheckCircle2, Circle, Square, CheckSquare, AlignLeft, ListOrdered,
  Calendar, ChevronRight as ChevronRightIcon, XCircle, PlayCircle, PauseCircle,
  UserX
} from 'lucide-react';
...
const SurveyProgress = styled.div`
  margin-top: 0.75rem;
  padding-top: 0.75rem;
...
const SurveyModalHeader = styled.div`
  display: flex;
  justify-content: space-between;
  align-items: center;
  padding: 1.5rem;
...
const SurveyModalTitle = styled.h2`
  font-size: 1.25rem;
  font-weight: 600;
  color: #1e293b;
  margin: 0;
...
const SurveyModalBody = styled.div`
  flex: 1;
  overflow-y: auto;
  padding: 1.5rem;
`;

const SurveyModalFooter = styled.div`
  display: flex;
  justify-content: flex-end;
  gap: 0.75rem;
  padding: 1rem 1.5rem;
...
const ShortTextAnswer = styled.input`
  width: calc(100% - 2rem);
  margin-left: 2rem;
  padding: 0.75rem 1rem;
  border: 1px solid #e2e8f0;
...
// Survey Form Styles (for creating/editing)
...
const OptionEditorInput = styled.input`
  flex: 1;
...
const TextResponseList = styled.div`
  max-height: 200px;
  overflow-y: auto;
  display: flex;
  flex-direction: column;
...
const TextResponseItem = styled.div`
  padding: 0.75rem;
  background: white;
  border-radius: 0.5rem;
  font-size: 0.875rem;
...
const virtualCategories = [
  ...
  { id: 'survey', name: '설문조사', slug: 'survey', icon: 'ClipboardList', color: '#8b5cf6', postCount: 0, isSurvey: true }
];
...
const CollaborationBoardApp = ({ onGoHome }) => {
  ...
  const [surveyModalMode, setSurveyModalMode] = useState('view'); // 'view' | 'respond' | 'create' | 'edit' | 'stats'
  ...
  const fetchSurveys = useCallback(async () => {
    setLoading(true);
    try {
      const data = await boardApi.getSurveys();
      setSurveys(data);
    } catch (error) {
  ...
  useEffect(() => {
    ...
    if (currentCat?.isSurvey) {
      fetchSurveys();
    } else {
      fetchPosts(1);
    }
  }, [fetchPosts, fetchSurveys, selectedCategory, categories]);

  // Survey handlers
  const handleOpenSurvey = async (survey) => {
    try {
      const fullSurvey = await boardApi.getSurvey(survey.id);
      setSelectedSurvey(fullSurvey);

      // 이미 응답한 경우 기존 응답 로드
      if (fullSurvey.hasResponded) {
        try {
          const myResponse = await boardApi.getMyResponse(survey.id);
          const loadedAnswers = {};
          (myResponse.answers || []).forEach(ans => {
            if (ans.selectedOptions && ans.selectedOptions.length > 0) {
              loadedAnswers[ans.questionId] = { selectedOptions: ans.selectedOptions };
            } else if (ans.scaleValue !== null && ans.scaleValue !== undefined) {
              loadedAnswers[ans.questionId] = { scaleValue: ans.scaleValue };
            } else if (ans.answerText) {
              loadedAnswers[ans.questionId] = { answerText: ans.answerText };
            }
          });
          setSurveyAnswers(loadedAnswers);
        } catch (e) {
          console.error('Failed to load my response:', e);
          setSurveyAnswers({});
        }
        setSurveyModalMode('view');
      } else {
        setSurveyAnswers({});
        setSurveyModalMode('respond');
      }
  ...
  const handleOpenSurveyStats = async (survey, e) => {
    e.stopPropagation();
    try {
      const stats = await boardApi.getSurveyStats(survey.id);
      setSelectedSurvey(stats);
      setSurveyModalMode('stats');
      setShowSurveyModal(true);
    } catch (error) {
  ...
  const handleEditSurvey = async (survey, e) => {
    e.stopPropagation();
    try {
      const fullSurvey = await boardApi.getSurvey(survey.id);
      setSelectedSurvey(fullSurvey);

      // 날짜 형식 변환 (datetime-local input용)
      ...
      setSurveyFormData({
        ...
        startDate: formatDateForInput(fullSurvey.startDate),
        endDate: formatDateForInput(fullSurvey.endDate),
        isAnonymous: fullSurvey.isAnonymous || false,
        allowMultipleResponses: fullSurvey.allowMultipleResponses || false,
  ...
  const handleSaveSurvey = async () => {
    ...
    if (surveyFormData.questions.length === 0) {
      alert('최소 1개의 문항을 추가해주세요.');
      return;
    }

    try {
      if (surveyModalMode === 'create') {
        await boardApi.createSurvey(surveyFormData);
      } else {
        await boardApi.updateSurvey(selectedSurvey.id, surveyFormData);
      }
  ...
  const handleDeleteSurvey = async (surveyId, e) => {
    ...
    try {
      await boardApi.deleteSurvey(surveyId);
      fetchSurveys();
    } catch (error) {
  ...
  const handleCloseSurvey = async (surveyId, e) => {
    e.stopPropagation();
    if (!window.confirm('설문을 마감하시겠습니까?')) return;

    try {
      await boardApi.closeSurvey(surveyId);
      fetchSurveys();
    } catch (error) {
  ...
  const handleExportSurveyCsv = async () => {
    ...
    try {
      await boardApi.exportSurveyCsv(selectedSurvey.id, selectedSurvey.title);
    } catch (error) {
  ...
  const handleSurveyAnswer = (questionId, value, type) => {
    setSurveyAnswers(prev => {
      if (type === 'multiple') {
        ...
        return { ...prev, [questionId]: { scaleValue: value } };
  ...
  const handleSubmitSurveyResponse = async () => {
    const answers = Object.entries(surveyAnswers).map(([questionId, answer]) => ({
      questionId: parseInt(questionId),
      ...answer
    }));

    // Check required questions
    const requiredQuestions = selectedSurvey.questions.filter(q => q.isRequired);
    for (const q of requiredQuestions) {
      const answer = surveyAnswers[q.id];
      if (!answer ||
          (!answer.answerText && (!answer.selectedOptions || answer.selectedOptions.length === 0) && !answer.scaleValue)) {
        alert(`필수 문항에 응답해주세요: ${q.questionText}`);
        return;
      }
    }

    try {
      await boardApi.submitSurveyResponse(selectedSurvey.id, answers);
      alert('응답이 제출되었습니다.');
      setShowSurveyModal(false);
      fetchSurveys();
    } catch (error) {
      console.error('Failed to submit response:', error);
      alert(error.message || '응답 제출에 실패했습니다.');
    }
  };

  const handleStartEditResponse = () => {
    setSurveyModalMode('edit-response');
  };

  const handleUpdateSurveyResponse = async () => {
    const answers = Object.entries(surveyAnswers).map(([questionId, answer]) => ({
      questionId: parseInt(questionId),
      ...answer
    }));
    ...
    try {
      await boardApi.updateMyResponse(selectedSurvey.id, answers);
      alert('응답이 수정되었습니다.');
      setShowSurveyModal(false);
      fetchSurveys();
    } catch (error) {
      console.error('Failed to update response:', error);
      alert(error.message || '응답 수정에 실패했습니다.');
    }
  };
  ...
  const isSurveyBoard = currentCategory?.isSurvey || currentCategory?.slug === 'survey';
  ...
  return (
    <Container>
            ...
            <>
              <BoardHeader>
                ...
                <BoardActions>
                  ...
                  {isSurveyBoard ? (
                    <Button className="primary" onClick={handleCreateSurvey}>
                      <Plus size={16} />
                      새 설문
                    </Button>
                  ) : (
                ...
                <SurveyList>
                  {loading ? (
                    ...
                    surveys.map(survey => {
                      ...
                      const statusIcon = {
                        active: <PlayCircle size={12} />,
                        ended: <XCircle size={12} />,
                        scheduled: <Calendar size={12} />,
                        closed: <PauseCircle size={12} />
                      };
                      const isAuthor = user?.id === survey.authorId;
                      const canManage = isAuthor || user?.isAdmin;

                      return (
                        <SurveyCard key={survey.id} onClick={() => handleOpenSurvey(survey)}>
                          <SurveyCardHeader>
                          ...
                          <SurveyMeta>
                            <span><User size={14} /> {survey.authorName}</span>
                            <span><FileText size={14} /> {survey.questionCount}문항</span>
                            <span><Users size={14} /> {survey.responseCount}명 응답</span>
                            {survey.endDate && (
                              <span><Calendar size={14} /> ~{new Date(survey.endDate).toLocaleDateString('ko-KR', { timeZone: 'Asia/Seoul' })}</span>
                            )}
                          </SurveyMeta>
                          {canManage && (
                            <SurveyProgress>
                              ...
                              <Button
                                className="secondary"
                                style={{ padding: '0.375rem 0.75rem', fontSize: '0.75rem' }}
                                onClick={(e) => handleOpenSurveyStats(survey, e)}
                              >
      ...
      {/* Survey Modal */}
      {showSurveyModal && (
        <SurveyModal onClick={() => setShowSurveyModal(false)}>
          <SurveyModalContent onClick={(e) => e.stopPropagation()}>
            <SurveyModalHeader>
              <SurveyModalTitle>
                {surveyModalMode === 'create' && '새 설문 만들기'}
                {surveyModalMode === 'edit' && '설문 수정'}
                {surveyModalMode === 'view' && selectedSurvey?.title}
                {surveyModalMode === 'respond' && selectedSurvey?.title}
                {surveyModalMode === 'stats' && `${selectedSurvey?.title} - 통계`}
              </SurveyModalTitle>
              <Button className="ghost" onClick={() => setShowSurveyModal(false)}>
                <X size={20} />
              </Button>
            </SurveyModalHeader>
            ...
            <SurveyModalBody>
              {/* Create/Edit Mode */}
              {(surveyModalMode === 'create' || surveyModalMode === 'edit') && (
                <>
                  <SurveyFormSection>
                    <SurveyFormLabel>설문 제목 *</SurveyFormLabel>
                    <SurveyFormInput
                      type="text"
                  ...
                  <SurveyFormSection>
                    <SurveyFormLabel>설문 설명</SurveyFormLabel>
                    <SurveyFormTextarea
                  ...
                  <SurveyFormSection>
                    <SurveyFormLabel>설문 기간</SurveyFormLabel>
                    <DateInputRow>
                      <DateInput
                        type="datetime-local"
                        value={surveyFormData.startDate}
                        onChange={(e) => setSurveyFormData({ ...surveyFormData, startDate: e.target.value })}
                      />
                      <span>~</span>
                      <DateInput
                        type="datetime-local"
                        value={surveyFormData.endDate}
                        onChange={(e) => setSurveyFormData({ ...surveyFormData, endDate: e.target.value })}
                      />
                    </DateInputRow>
                  </SurveyFormSection>
                  ...
                  <SurveyFormSection>
                    <CheckboxRow>
                      <input
                        type="checkbox"
                        checked={surveyFormData.isAnonymous}
                        onChange={(e) => setSurveyFormData({ ...surveyFormData, isAnonymous: e.target.checked })}
                      />
                      익명 설문
                    </CheckboxRow>
                    <CheckboxRow style={{ marginTop: '0.5rem' }}>
                      <input
                        type="checkbox"
                        checked={surveyFormData.allowMultipleResponses}
                        onChange={(e) => setSurveyFormData({ ...surveyFormData, allowMultipleResponses: e.target.checked })}
                      />
                      중복 응답 허용
                    </CheckboxRow>
                  </SurveyFormSection>
                  ...
                  <SurveyFormSection>
                    <SurveyFormLabel>문항 목록</SurveyFormLabel>
                    {surveyFormData.questions.map((question, qIndex) => (
                      <QuestionEditor key={qIndex}>
                        <QuestionEditorHeader>
                          <span style={{ fontWeight: 500, color: '#374151' }}>문항 {qIndex + 1}</span>
                        ...
                        <CheckboxRow style={{ marginBottom: '0.75rem' }}>
                          <input
                            ...
                            onChange={(e) => handleUpdateQuestion(qIndex, 'isRequired', e.target.checked)}
                          />
                          필수 응답
                        </CheckboxRow>
                        ...
                        {['single', 'multiple'].includes(question.questionType) && (
                          <div>
                            ...
                            <AddOptionButton onClick={() => handleAddOption(qIndex)}>
                              <Plus size={14} /> 옵션 추가
                            </AddOptionButton>
                          </div>
                        )}
                        ...
                        {question.questionType === 'scale' && (
                          <div style={{ display: 'flex', gap: '1rem', alignItems: 'center' }}>
                            <div>
                              <label style={{ fontSize: '0.75rem', color: '#64748b' }}>최소값</label>
                              <SurveyFormInput
                                type="number"
                                value={question.scaleMin}
                                onChange={(e) => handleUpdateQuestion(qIndex, 'scaleMin', parseInt(e.target.value))}
                                style={{ width: '80px' }}
                              />
                            </div>
                            <div>
                              <label style={{ fontSize: '0.75rem', color: '#64748b' }}>최대값</label>
                              <SurveyFormInput
                                type="number"
                                value={question.scaleMax}
                                onChange={(e) => handleUpdateQuestion(qIndex, 'scaleMax', parseInt(e.target.value))}
                                style={{ width: '80px' }}
                              />
                            </div>
                            <div style={{ flex: 1 }}>
                              <label style={{ fontSize: '0.75rem', color: '#64748b' }}>최소 라벨</label>
                              <SurveyFormInput
                                type="text"
                            ...
                            <div style={{ flex: 1 }}>
                              <label style={{ fontSize: '0.75rem', color: '#64748b' }}>최대 라벨</label>
                              <SurveyFormInput
                                type="text"
              ...
              {/* View/Respond/Edit-Response Mode */}
              {(surveyModalMode === 'view' || surveyModalMode === 'respond' || surveyModalMode === 'edit-response') && selectedSurvey && (
                <>
                  {selectedSurvey.description && (
                    <SurveyDescription style={{ marginBottom: '1.5rem' }}>
                      {selectedSurvey.description}
                    </SurveyDescription>
                  )}

                  {selectedSurvey.hasResponded && surveyModalMode === 'view' && (
                    <div style={{ padding: '1rem', background: '#f0f9ff', borderRadius: '0.5rem', marginBottom: '1.5rem', display: 'flex', alignItems: 'center', gap: '0.5rem', color: '#0369a1' }}>
                      <CheckCircle2 size={20} />
                      이미 응답을 완료한 설문입니다.
                    </div>
                  )}

                  {surveyModalMode === 'edit-response' && (
                    <div style={{ padding: '1rem', background: '#fef3c7', borderRadius: '0.5rem', marginBottom: '1.5rem', display: 'flex', alignItems: 'center', gap: '0.5rem', color: '#92400e' }}>
                      <Edit2 size={20} />
                      응답을 수정하고 있습니다.
                    </div>
                  )}
                        ...
                        <QuestionCard key={question.id}>
                            ...
                            <OptionList>
                              {(question.options || []).map((opt, oIndex) => (
                                <OptionItem
                                  ...
                                  onClick={() => (surveyModalMode === 'respond' || surveyModalMode === 'edit-response') && handleSurveyAnswer(question.id, opt, 'single')}
                                >
                                  <OptionRadio $selected={selectedOptions.includes(opt)} />
                            ...
                            <OptionList>
                              {(question.options || []).map((opt, oIndex) => (
                                <OptionItem
                                  key={oIndex}
                                  $selected={selectedOptions.includes(opt)}
                                  onClick={() => (surveyModalMode === 'respond' || surveyModalMode === 'edit-response') && handleSurveyAnswer(question.id, opt, 'multiple')}
                                >
                                  <OptionCheckbox $selected={selectedOptions.includes(opt)}>
                          ...
                          {question.questionType === 'text' && (
                            <ShortTextAnswer
                              ...
                              onChange={(e) => (surveyModalMode === 'respond' || surveyModalMode === 'edit-response') && handleSurveyAnswer(question.id, e.target.value, 'text')}
                              placeholder="답변을 입력하세요"
                              disabled={surveyModalMode === 'view'}
                            />
                          )}
                          ...
                          {question.questionType === 'textarea' && (
                            <TextAnswer
                              value={answer?.answerText || ''}
                              onChange={(e) => (surveyModalMode === 'respond' || surveyModalMode === 'edit-response') && handleSurveyAnswer(question.id, e.target.value, 'textarea')}
                              placeholder="답변을 입력하세요"
                              disabled={surveyModalMode === 'view'}
                            />
                          )}
                            ...
                            <ScaleContainer>
                              ...
                              <ScaleOptions>
                                {Array.from(
                                  ...
                                  <ScaleOption
                                    key={val}
                                    $selected={answer?.scaleValue === val}
                                    onClick={() => (surveyModalMode === 'respond' || surveyModalMode === 'edit-response') && handleSurveyAnswer(question.id, val, 'scale')}
                                    disabled={surveyModalMode === 'view'}
                                  >
                                    {val}
                                  </ScaleOption>
              ...
              {/* Stats Mode */}
              {surveyModalMode === 'stats' && selectedSurvey && selectedSurvey.stats && (
                <StatsContainer>
                  <div style={{ padding: '1rem', background: '#f0f9ff', borderRadius: '0.5rem', marginBottom: '1rem' }}>
                    <strong>총 응답 수:</strong> {selectedSurvey.stats.totalResponses}명
                  </div>

                  {selectedSurvey.stats.questions.map((stat, index) => (
                    <StatCard key={stat.questionId}>
                      <StatHeader>
                        <StatQuestion>Q{index + 1}. {stat.questionText}</StatQuestion>
                        <StatType>{
                          { single: '단일선택', multiple: '복수선택', text: '단답형', textarea: '서술형', scale: '척도' }[stat.questionType]
                        }</StatType>
                      </StatHeader>

                      {['single', 'multiple', 'dropdown'].includes(stat.questionType) && (
                        <BarChartContainer>
                          {stat.answers.map((ans, aIndex) => {
                            const total = selectedSurvey.stats.totalResponses || 1;
                            const percent = Math.round((ans.count / total) * 100);
                            return (
                              <BarChartRow key={aIndex}>
                                <BarChartLabel title={ans.option}>{ans.option}</BarChartLabel>
                                <BarChartBarContainer>
                                  <BarChartBar $percent={percent} />
                                </BarChartBarContainer>
                                <BarChartValue>{ans.count}명 ({percent}%)</BarChartValue>
                              </BarChartRow>
                            );
                          })}
                        </BarChartContainer>
                      ...
                      {stat.questionType === 'scale' && (
                        <>
                          {stat.average !== undefined && (
                            <AverageDisplay>
                              <AverageValue>{stat.average}</AverageValue>
                              <AverageLabel>평균 점수</AverageLabel>
                            </AverageDisplay>
                          )}
                          <BarChartContainer style={{ marginTop: '1rem' }}>
                            {stat.answers.map((ans, aIndex) => {
                              const total = selectedSurvey.stats.totalResponses || 1;
                              const percent = Math.round((ans.count / total) * 100);
                              return (
                                <BarChartRow key={aIndex}>
                                  <BarChartLabel>{ans.value}점</BarChartLabel>
                                  <BarChartBarContainer>
                                    <BarChartBar $percent={percent} />
                                  </BarChartBarContainer>
                                  <BarChartValue>{ans.count}명 ({percent}%)</BarChartValue>
                      ...
                      {['text', 'textarea'].includes(stat.questionType) && (
                        <TextResponseList>
                          {stat.answers.length === 0 ? (
                            <div style={{ color: '#94a3b8', textAlign: 'center', padding: '1rem' }}>응답 없음</div>
                          ) : (
                            stat.answers.map((ans, aIndex) => (
                              <TextResponseItem key={aIndex}>{ans.text}</TextResponseItem>
                            ))
                          )}
                        </TextResponseList>
                      )}
            ...
            <SurveyModalFooter>
              {(surveyModalMode === 'create' || surveyModalMode === 'edit') && (
                <>
                  <Button className="secondary" onClick={() => setShowSurveyModal(false)}>취소</Button>
                  <Button className="primary" onClick={handleSaveSurvey}>
                    {surveyModalMode === 'create' ? '설문 만들기' : '저장'}
                  </Button>
                </>
              )}
              {surveyModalMode === 'respond' && (
                <>
                  <Button className="secondary" onClick={() => setShowSurveyModal(false)}>취소</Button>
                  <Button className="primary" onClick={handleSubmitSurveyResponse}>응답 제출</Button>
                </>
              )}
              {surveyModalMode === 'view' && (
                <>
                  {selectedSurvey?.hasResponded && selectedSurvey?.status === 'active' && (
                    <Button className="primary" onClick={handleStartEditResponse}>
                      <Edit2 size={16} /> 응답 수정
                    </Button>
                  )}
                  <Button className="secondary" onClick={() => setShowSurveyModal(false)}>닫기</Button>
                </>
              )}
              {surveyModalMode === 'edit-response' && (
                <>
                  <Button className="secondary" onClick={() => setSurveyModalMode('view')}>취소</Button>
                  <Button className="primary" onClick={handleUpdateSurveyResponse}>수정 완료</Button>
                </>
              )}
              {surveyModalMode === 'stats' && (
                <>
                  <Button className="primary" onClick={handleExportSurveyCsv}>
