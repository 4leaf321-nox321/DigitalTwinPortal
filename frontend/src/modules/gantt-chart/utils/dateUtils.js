 ...
 *
 * ⚠️ 예전에는 `toISOString().split('T')[0]`(UTC)이었다. 그러면 `new Date()` 처럼
 *    **지금**에서 만든 날짜가 KST 새벽 0~9시에 하루 밀린다. 로컬 기준으로 바꿨다.
 *    (shared/utils/localDate.js 참조)
 */
    ...
    if (monthDates.length > 0) {
      ...
      for (let i = 0; i < monthDates.length; i += 7) {
        ...
        if (currentMonth === endMonth && currentYear === endYear) {
          console.log(`${currentYear}.${String(currentMonth + 1).padStart(2, '0')} 월 주 생성: ${weekStart.toLocaleDateString('ko-KR', { timeZone: 'Asia/Seoul' })} ~ ${weekEnd.toLocaleDateString('ko-KR', { timeZone: 'Asia/Seoul' })} = ${weekData.year}년 ${weekData.weekNumber}주`);
          console.log(`  주에 포함된 날짜들:`, weekDates.map(d => d.toLocaleDateString('ko-KR', { timeZone: 'Asia/Seoul' })).join(', '));
          console.log(`  프로젝트 종료일: ${rangeForMonths.end.toLocaleDateString('ko-KR', { timeZone: 'Asia/Seoul' })}`);
        }
        ...
        if (!belongsToCurrentMonth) {
          console.log(`다른 월 소속 주 제외: ${weekStart.toLocaleDateString('ko-KR', { timeZone: 'Asia/Seoul' })} ~ ${weekEnd.toLocaleDateString('ko-KR', { timeZone: 'Asia/Seoul' })} = ${weekData.year}년 ${weekData.weekNumber}주`);
          continue;
        }
...
export const getProjectDateRange = (tasks) => {
  ...
  const dates = tasks.reduce((acc, task) => {
    if (task.startDate) {
      ...
      console.log(`태스크 "${task.name}" 시작일: ${task.startDate} -> ${startDate.toLocaleDateString('ko-KR', { timeZone: 'Asia/Seoul' })}`);
      acc.push(startDate);
    }
    if (task.endDate) {
      const endDate = normalizeDate(task.endDate);
      console.log(`태스크 "${task.name}" 종료일: ${task.endDate} -> ${endDate.toLocaleDateString('ko-KR', { timeZone: 'Asia/Seoul' })}`);
      acc.push(endDate);
    }
    return acc;
  }, []);
  ...
  console.log('실제 시작일:', start.toLocaleDateString('ko-KR', { timeZone: 'Asia/Seoul' }));
  console.log('실제 종료일:', end.toLocaleDateString('ko-KR', { timeZone: 'Asia/Seoul' }));
  console.log('============================================');
...
export const getGridDateRange = (displayRange) => {
  ...
  console.log('사용자 범위:', displayRange.start.toLocaleDateString('ko-KR', { timeZone: 'Asia/Seoul' }), '~', displayRange.end.toLocaleDateString('ko-KR', { timeZone: 'Asia/Seoul' }));
  console.log('그리드 범위 (주 확장):', expandedStart.toLocaleDateString('ko-KR', { timeZone: 'Asia/Seoul' }), '~', expandedEnd.toLocaleDateString('ko-KR', { timeZone: 'Asia/Seoul' }));
  console.log('========================');
...
export const debugDateRange = (startDate, endDate, viewType = 'all') => {
  console.log('\n=== 날짜 범위 디버깅 ===');
  console.log('시작일:', startDate.toLocaleDateString('ko-KR', { timeZone: 'Asia/Seoul' }));
  console.log('종료일:', endDate.toLocaleDateString('ko-KR', { timeZone: 'Asia/Seoul' }));
  ...
  if (viewType === 'all' || viewType === 'weeks') {
    ...
    weeks.forEach((week, i) => {
      console.log(`${i + 1}. ${week.label}: ${week.start.toLocaleDateString('ko-KR', { timeZone: 'Asia/Seoul' })} ~ ${week.end.toLocaleDateString('ko-KR', { timeZone: 'Asia/Seoul' })} (${week.dates.length}일)`);
    });
  }
  ...
  if (viewType === 'all' || viewType === 'months') {
    ...
    months.forEach((month, i) => {
      console.log(`${i + 1}. ${month.label}: ${month.start.toLocaleDateString('ko-KR', { timeZone: 'Asia/Seoul' })} ~ ${month.end.toLocaleDateString('ko-KR', { timeZone: 'Asia/Seoul' })}`);
      console.log(`     실제 월: ${month.actualStart.toLocaleDateString('ko-KR', { timeZone: 'Asia/Seoul' })} ~ ${month.actualEnd.toLocaleDateString('ko-KR', { timeZone: 'Asia/Seoul' })}`);
  ...
  if (viewType === 'all' || viewType === 'years') {
    ...
    years.forEach((year, i) => {
      console.log(`${i + 1}. ${year.label}: ${year.start.toLocaleDateString('ko-KR', { timeZone: 'Asia/Seoul' })} ~ ${year.end.toLocaleDateString('ko-KR', { timeZone: 'Asia/Seoul' })}`);
      console.log(`     실제 연도: ${year.actualStart.toLocaleDateString('ko-KR', { timeZone: 'Asia/Seoul' })} ~ ${year.actualEnd.toLocaleDateString('ko-KR', { timeZone: 'Asia/Seoul' })}`);
