...
/** 그 칸에 있는 사업부들. 레이더 점에 붙는 이름표와 **같은 것**이어야 한다. */
 ...
 *
 * ⚠️ 한 역량이 **여러 칸에 선다** — 사업부마다 다르게 적었으면 그게 맞는 그림이다.
 *    레이더에서 점이 갈리는 것과 같은 일이다.
 */
const inColumn = (rows, key) => {
  const out = [];
  rows.forEach((t) => {
    const marks = t.divisionMarks || [];
    if (marks.length) {
      const mine = marks.filter((m) => m.stage === key);
...
const RadarBoard = ({ rows, onSelect }) => (
  <Board>
    {[...STAGES, NOBODY].map((st) => {
      const items = inColumn(rows, st.key);
      return (
        <Column key={st.key} $bg={st.bg} $border={st.border}>
          ...
          {items.map(({ tech: t, divisions }) => (
            <Item key={`${t.uuid}|${st.key}`} onClick={() => onSelect(t)}>
              <Row>
                <b>{t.name}</b>
                {/* ⚠️ **누가 여기라고 했는지**가 이 줄의 알맹이다 — 이름 바로 뒤다. */}
                {divisions.length > 0 && <Who>{divisions.join(' · ')}</Who>}
