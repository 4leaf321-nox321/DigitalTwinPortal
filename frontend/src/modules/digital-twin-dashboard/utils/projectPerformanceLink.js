...
import { hasLevel } from './levelValue';

/**
 * 성과 ID 또는 UUID로 성과 찾기 (하위 호환성 지원)
 * - 기존 데이터: id로 연결 (예: "performance-1")
 * - 신규 데이터: uuid로 연결 (예: "abc-123-xxx")
 * @param {string} identifier - 성과 ID 또는 UUID
 * @param {Array} performances - 성과 목록
 * @returns {Object|null} 찾은 성과 객체 또는 null
 */
export const findPerformanceByIdOrUuid = (identifier, performances) => {
  if (!identifier || !performances) return null;
  return performances.find(p => p.id === identifier || p.uuid === identifier);
};

/**
 * 성과 목록에서 특정 ID/UUID가 포함되어 있는지 확인
 * @param {Array} performanceRefs - 성과 참조 배열 (ID 또는 객체)
 * @param {Object} performance - 확인할 성과 객체
 * @returns {boolean}
 */
export const hasPerformanceLink = (performanceRefs, performance) => {
  if (!performanceRefs || !Array.isArray(performanceRefs) || !performance) return false;

  return performanceRefs.some(ref => {
    const refId = typeof ref === 'string' ? ref : (ref.id || ref.성과항목ID);
    return refId === performance.id || refId === performance.uuid;
  });
};
...
export const getProjectPerformancesWithData = (project, globalPerformances = null) => {
  ...
  return project.성과목록.map(perfRef => {
    // perfRef는 문자열(ID 또는 UUID) 또는 객체일 수 있음
    // 객체인 경우: {id, 과제기여도, 실적수준} 또는 {성과항목ID, 성과항목UUID, 과제기여도, 실적수준}
    // UUID 우선 사용 (신규 데이터), 없으면 ID 사용 (하위 호환)
    const performanceIdentifier = typeof perfRef === 'string'
      ? perfRef
      : (perfRef.성과항목UUID || perfRef.id || perfRef.성과항목ID);
    const contribution = typeof perfRef === 'object' ? perfRef.과제기여도 : '100';
    const actualLevel = typeof perfRef === 'object' ? perfRef.실적수준 : '';

    // 글로벌 성과 목록에서 해당 성과 찾기 (ID 또는 UUID로 검색)
    const performanceData = findPerformanceByIdOrUuid(performanceIdentifier, performances);

    if (!performanceData) {
      // 경고 로그 제거 - cleanup 함수에서 이미 고아 참조를 감지하고 처리하므로 중복 경고 불필요
      // React concurrent mode와 StrictMode에서 타이밍 이슈로 혼란스러운 경고가 발생할 수 있음
      return null;
    }
 ...
 * @param {string|Object} performanceIdOrObj - 성과 항목 ID/UUID 또는 성과 객체
 * @param {Array} projects - 프로젝트 배열 (선택사항)
 * @returns {Array} 해당 성과를 사용하는 프로젝트 배열
 */
export const getProjectsUsingPerformance = (performanceIdOrObj, projects = null) => {
  const projectList = projects || loadProjects();

  // 성과 객체인 경우 id와 uuid 둘 다 비교, 문자열인 경우 그대로 사용
  const perfId = typeof performanceIdOrObj === 'object' ? performanceIdOrObj.id : performanceIdOrObj;
  const perfUuid = typeof performanceIdOrObj === 'object' ? performanceIdOrObj.uuid : performanceIdOrObj;
  ...
  return projectList.filter(project => {
    ...
    return project.성과목록.some(perfRef => {
      // 문자열이면 그대로 사용, 객체면 UUID 우선 확인
      const refId = typeof perfRef === 'string'
        ? perfRef
        : (perfRef.성과항목UUID || perfRef.id || perfRef.성과항목ID);
      return refId === perfId || refId === perfUuid;
    });
  });
};
...
export const calculateProjectPerformanceStats = (project, globalPerformances = null) => {
  ...
  performances.forEach(perf => {
    // 실적 입력 완료 여부. **0 도 입력된 것이다** (levelValue.js 참조).
    // `.trim()` 을 직접 부르면 안 된다 — 숫자로 들어오는 경로가 있어 터진다.
    if (hasLevel(perf.실적수준) && String(perf.실적수준).trim() !== '') {
      stats.실적입력완료수++;
...
/**
 * 프로젝트의 고아 성과 참조 정리 (존재하지 않는 성과 ID 제거)
 * @param {Array} projects - 프로젝트 배열
 * @param {Array} globalPerformances - 글로벌 성과 목록
 * @returns {Object} { cleanedProjects, removedCount, affectedProjects }
 */
export const cleanupOrphanedPerformanceLinks = (projects, globalPerformances) => {
  if (!projects || !globalPerformances) {
    return { cleanedProjects: projects || [], removedCount: 0, affectedProjects: [] };
  }
  ...
  const cleanedProjects = projects.map(project => {
    ...
    const originalLength = project.성과목록.length;
    const cleanedPerformanceList = project.성과목록.filter(perfRef => {
      const performanceIdentifier = typeof perfRef === 'string'
        ? perfRef
        : (perfRef.성과항목UUID || perfRef.id || perfRef.성과항목ID);

      return findPerformanceByIdOrUuid(performanceIdentifier, globalPerformances) !== null;
    });

    const removedFromProject = originalLength - cleanedPerformanceList.length;
    if (removedFromProject > 0) {
      removedCount += removedFromProject;
    ...
    return {
      ...project,
      성과목록: cleanedPerformanceList
    };
  });

  return { cleanedProjects, removedCount, affectedProjects };
};
