...
import { saveErrorMessage, fetchPerformanceDeleteSummary } from '../../services/dashboardWriteApi';
...
const AddPerformanceModal = ({
  ...
  // '전체 삭제'를 **서버까지** 수행하는 경로. 넘기면 이 버튼이 실제 삭제가 된다.
  //
  // 이 모달은 세 곳에서 쓰이는데 `globalPerformances` 의 뜻이 다르다 —
  // 대시보드에서는 **전역 성과 목록**이지만, 과제 추가·수정 모달에서는
  // **그 과제의 성과 목록**(localPerformances)이다. 후자에서 '전체 삭제'는
  // "이 과제의 연결을 비운다" 는 뜻이라 서버를 지우면 안 된다.
  // 그래서 대시보드에서만 이 props 를 넘기고, 없으면 예전처럼 로컬만 비운다.
  onDeleteAll = null,
  settingsData = {},
  globalPerformances = [],
  showSuccess,
  ...
  // 전체 삭제 확인 모달 상태
  //
  // `summary` 는 **서버가 센 실제 대상 수**다. 화면 목록으로 세지 않는 이유 —
  // 이 목록에는 연도 필터가 걸려 있을 수 있고 모달을 열어둔 사이 남이 추가했을
  // 수도 있다. 본 숫자와 지울 숫자가 다르면 잘못된 근거로 누르게 된다.
  const [deleteAllConfirm, setDeleteAllConfirm] = useState({
    isOpen: false,
    loading: false,
    summary: null,      // { activeCount, affectedProjectCount }
    error: null,
    deleting: false
  });
  ...
  const handleEditPerformance = (performance) => {
    ...
    setFormData({
      ...
      소분류: performance.소분류 || '',
      성과항목: extractedName,
      // ⚠️ `|| ''` 를 쓰면 안 된다 — 저장된 **0 이 미입력으로 바뀌어** 빈칸으로 열리고,
      //    그대로 저장하면 0 이 지워진다. 서버는 현재/목표를 숫자 0 으로 주므로
      //    `String(0)` 이 되게 levelText 를 쓴다. (2026-08-06)
      현재수준: levelText(performance.현재수준),
      목표수준: levelText(performance.목표수준),
      로직입력여부: performance.로직입력여부 || false,
      월별실적여부: performance.월별실적여부 || false,
      실적수준: levelText(performance.실적수준),
  ...
  const handleDeleteAllClick = async () => {
    ...
    // 로컬 경로(과제 모달)는 서버에 물어볼 것이 없다.
    if (!onDeleteAll) {
      setDeleteAllConfirm({ isOpen: true, loading: false, summary: null,
                            error: null, deleting: false });
      return;
    }

    setDeleteAllConfirm({ isOpen: true, loading: true, summary: null,
                          error: null, deleting: false });
    try {
      const summary = await fetchPerformanceDeleteSummary();
      setDeleteAllConfirm(prev => ({ ...prev, loading: false, summary }));
    } catch (error) {
      // 대상 수를 못 읽으면 **삭제 버튼을 열지 않는다** — 몇 건이 지워지는지
      // 모르는 채로 누르게 하는 것이 이 확인 절차가 막으려던 바로 그 상황이다.
      setDeleteAllConfirm(prev => ({
        ...prev, loading: false,
        error: error.message || '삭제 대상 건수를 불러오지 못했습니다.'
      }));
    }
  };
  ...
  const handleConfirmDeleteAll = async () => {
    // ── 로컬 경로 (과제 추가·수정 모달) ─────────────────────────────────
    // 여기서 '전체 삭제'는 "이 과제의 성과 연결을 비운다" 는 뜻이다. 예전 그대로.
    if (!onDeleteAll) {
      try {
        const totalCount = globalPerformances.length;
        if (onDelete) onDelete('ALL');
        showSuccess(`성과 항목 ${totalCount}개를 목록에서 제거했습니다.`);
        if (editingPerformance) handleFormReset();
      } catch (error) {
        showError('성과 목록 비우기 중 오류가 발생했습니다: ' + error.message);
      }
      setDeleteAllConfirm({ isOpen: false, loading: false, summary: null,
                            error: null, deleting: false });
      return;
    }

    // ── 서버 경로 (대시보드) ────────────────────────────────────────────
    setDeleteAllConfirm(prev => ({ ...prev, deleting: true, error: null }));
    try {
      // 방금 읽은 서버 건수를 그대로 되돌려준다. 그 사이 또 바뀌었으면 서버가 409.
      const result = await onDeleteAll({
        expectedCount: deleteAllConfirm.summary?.activeCount
      });
      const count = result?.count ?? 0;
      const affected = result?.affectedProjectCount ?? 0;
      showSuccess(`성과 ${count}개를 삭제했습니다.`
                  + (affected ? ` 연결이 해제된 과제 ${affected}건.` : ''));
      if (editingPerformance) handleFormReset();
      setDeleteAllConfirm({ isOpen: false, loading: false, summary: null,
                            error: null, deleting: false });
    } catch (error) {
      // 409(그 사이 건수가 바뀜)는 다시 열어 새 숫자를 보여주는 게 맞다.
      setDeleteAllConfirm(prev => ({
        ...prev, deleting: false,
        error: error.message || '성과 전체 삭제에 실패했습니다.'
      }));
    }
  };
  ...
  const handleCancelDeleteAll = () => {
    if (deleteAllConfirm.deleting) return;   // 지우는 중에는 닫지 않는다
    setDeleteAllConfirm({ isOpen: false, loading: false, summary: null,
                          error: null, deleting: false });
  };
              ...
              <div className="panel-header">
                ...
                {isAdmin && globalPerformances.length > 0 && (
                  <button
                    ...
                    title={onDeleteAll
                      ? '등록된 모든 성과를 서버에서 삭제 (관리자 전용)'
                      : '이 과제의 성과 목록만 비웁니다'}
                  >
                    <Trash2 size={16} />
                    {onDeleteAll ? '전체 삭제' : '목록 비우기'}
                  </button>
                )}
              </div>
                      ...
                      <tbody>
                        {filteredPerformances.map((performance, index) => (
                          ...
                          >
                            ...
                            <td className="perf-table-td-num">{levelText(performance.현재수준, '-')}</td>
                            <td className="perf-table-td-num">{levelText(performance.목표수준, '-')}</td>
                            <td className="perf-table-td-num">
                              {performance.월별실적여부 ? '월별' : levelText(performance.실적수준, '-')}
                            </td>
                  ...
                  filteredPerformances.map((performance, index) => (
                    ...
                    >
                      ...
                      <div className="card-content">
                        ...
                        {(hasLevel(performance.현재수준) || hasLevel(performance.목표수준)
                          || hasLevel(performance.실적수준) || performance.월별실적여부) && (
                          <div className="performance-levels">
                            {hasLevel(performance.현재수준) && (
                              <span className="level-item">
                                {getPerformanceCurrentLabel(performance)}: {performance.현재수준} {performance.단위}
                              </span>
                            )}
                            {hasLevel(performance.목표수준) && (
                              <span className="level-item">
          ...
          {deleteAllConfirm.isOpen && (
            ...
            >
              ...
              >
                <div className="delete-modal-header">
                  <AlertTriangle size={24} className="warning-icon" />
                  <h3>{onDeleteAll ? '성과 전체 삭제' : '성과 목록 비우기'}</h3>
                </div>

                <div className="delete-modal-content">
                  {!onDeleteAll ? (
                    <>
                      {/* 과제 모달 — 이 과제의 연결만 비운다. 서버는 건드리지 않는다. */}
                      <p>이 과제의 성과 목록을 비우시겠습니까?</p>
                      <div className="delete-target">
                        <strong>대상: 이 과제의 성과 {globalPerformances.length}개</strong>
                        <div className="delete-target-detail">
                          성과 자체는 지워지지 않고 이 과제에서만 빠집니다
                        </div>
                      </div>
                    </>
                  ) : deleteAllConfirm.loading ? (
                    <p>삭제 대상을 확인하는 중입니다…</p>
                  ) : deleteAllConfirm.error ? (
                    <div className="warning-message">
                      <AlertTriangle size={16} />
                      <span>{deleteAllConfirm.error}</span>
                    </div>
                  ) : (
                    <>
                      <p>등록된 <strong>모든</strong> 성과 항목을 삭제하시겠습니까?</p>

                      <div className="delete-target">
                        <strong>
                          대상: 전체 {deleteAllConfirm.summary?.activeCount ?? 0}개 성과 항목
                        </strong>
                        <div className="delete-target-detail">
                          연결이 해제되는 과제{' '}
                          {deleteAllConfirm.summary?.affectedProjectCount ?? 0}건
                        </div>
                      </div>
                      ...
                      <div className="warning-message">
                        <AlertTriangle size={16} />
                        <span>
                          ⚠️ 서버에서 삭제됩니다. 이 성과들을 쓰던 <strong>모든 과제의
                          연결이 함께 사라지고, 성과를 되살려도 연결은 돌아오지
                          않습니다.</strong>
                        </span>
                      </div>
                    </>
                ...
                <div className="delete-modal-actions">
                  ...
                  <button
                    ...
                    className="confirm-delete-btn"
                    // 대상 수를 못 읽었으면 누를 수 없다 — 몇 건이 지워지는지
                    // 모르는 채로 확인시키지 않는다.
                    disabled={
                      deleteAllConfirm.deleting ||
                      (!!onDeleteAll && (deleteAllConfirm.loading ||
                                         !deleteAllConfirm.summary))
                    }
                  >
                    <Trash2 size={16} />
                    {deleteAllConfirm.deleting
