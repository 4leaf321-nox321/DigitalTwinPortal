...
// 액션 아이템의 완료 상태 계산 (세부 항목 기반)
const calculateActionItemCompletion = (actionItem) => {
  const detailItems = actionItem.세부항목목록 || [];
  ...
  if (detailItems.length > 0) {
    ...
    return {
      ...
      rate: (completedDetails / detailItems.length) * 100
    };
  }
  ...
  return {
    total: 1,
    completed: actionItem.완료여부 ? 1 : 0,
    rate: actionItem.완료여부 ? 100 : 0
  };
};
...
const ActionItemsStatus = ({ projects, statusColors }) => {
  ...
  const actionItemsData = useMemo(() => {
    ...
    projects.forEach(project => {
      ...
      if (items.length > 0) {
        ...
        // 각 액션 아이템별로 세부 항목 기반 계산
        items.forEach(item => {
          const completion = calculateActionItemCompletion(item);
          projectTotal += completion.total;
          projectCompleted += completion.completed;
          projectPlanned += (completion.total - completion.completed);
        });

        const rate = projectTotal > 0 ? (projectCompleted / projectTotal * 100) : 0;
        ...
        projectsWithItems.push({
          ...
          completedItems: projectCompleted,
          plannedItems: projectPlanned,
          completionRate: rate
        });
  ...
  const stats = [
    ...
    {
      value: actionItemsData.plannedItems,
      label: '계획 아이템',
      bgColor: '#fffbeb',
      borderColor: '#fde68a',
      valueColor: '#d97706'
            ...
            return (
              ...
              >
                <ProjectInfo>
                  ...
                  <ProjectStats>
                    {project.division} • 총 {project.totalItems} (계획 {project.plannedItems} / 완료 {project.completedItems})
                  </ProjectStats>
                </ProjectInfo>
                <ProjectBadge 
                  color={badgeColor} 
