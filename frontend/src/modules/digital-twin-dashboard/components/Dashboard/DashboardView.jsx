...
import { getActionItemCreatedAt } from '../../utils/actionItemTime';
import { makeExecMetrics, projectRemoval, aiExistedAt } from '../../utils/execMetrics';
...
const DashboardView = ({
  ...
  /**
   * 숫자 계산 세 벌. 정의는 utils/execMetrics 에 있다.
   *
   * ⚠️ **이력을 물리는 자리가 여기 한 곳뿐이다.** 함수마다 인자로 넘기는
   *    방식이었다면 넘기는 곳 하나만 빠뜨려도 조용히 틀렸을 텐데, 이렇게
   *    묶어 두면 그럴 자리가 없다. 아래 이름들은 예전과 **똑같아서** 쓰는 쪽
   *    코드는 한 줄도 안 바뀐다.
   */
  const { aiCountsAt, aiCountProgressAsOf, computeExecMetrics } = useMemo(
