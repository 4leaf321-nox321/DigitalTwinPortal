...
import { fetchWorklist } from './services/worklistApi';
...
import OwnerLinkAuditModal from './components/OwnerAudit/OwnerLinkAuditModal';
import MailRecipientModal from './components/MailRecipients/MailRecipientModal';
...
/*
  지식 그래프 내보내기·저장을 내렸다 (2026-08-09) — 「관계도」 탭이 대신한다.
  · 로컬 JSON  `exportKnowledgeGraph`
  · 서버 저장  `generateKnowledgeGraphData` / `generatePersonnelKnowledgeGraphData`
               + dx-work-process 의 `createGraph`/`saveGraphData`
  ⚠️ `createGraph`·`saveGraphData` 함수 자체는 **안 지웠다.** dx-work-process 모듈이
     자기 화면에서 계속 쓴다 — 여기서는 import 만 끊는다.
*/
...
/**
 * ⚠️ **`align-items: center` 는 내용이 화면보다 길면 위쪽을 잘라먹는다.**
 *
 * 이 상자는 `display:flex` + `overflow-y:auto` 인데, 세로 가운데 정렬이 걸린 채로
 * 자식이 상자보다 길어지면 **자식의 윗부분이 스크롤 영역 밖(음수 위치)으로 올라가
 * 스크롤로도 못 내려온다.** 헤더가 덮은 것처럼 보이지만 실은 잘린 것이다.
 *
 * 그래서 **긴 내용을 그리는 뷰는 반드시 `stretch` 여야 한다.** 새 뷰를 추가할 때
 * 아래 목록에 넣는 것을 잊지 말 것 (2026-08-11 「내 업무」 추가하며 실제로 물렸다).
 */
const STRETCH_VIEWS = ['dashboard', 'worklist'];

const Content = styled.div`
  flex: 1;
  padding: ${props => props.viewMode === 'gantt' ? '1rem' : '0'};
  display: flex;
  align-items: ${props => (STRETCH_VIEWS.includes(props.viewMode) ? 'stretch' : 'center')};
...
const DigitalTwinDashboardApp = () => {
  ...
  //
  // ⚠️ **2026-08-11: 이 팝업은 「내 일」 드로어에 흡수됐다.** 판정이 서버로 갔다 —
  //    여기 있던 `과제PL || 작성자` **이름 문자열 비교**는 서버가 knoxId 로만
  //    판정하는 규칙과 어긋났다(같은 상태를 두 곳이 다르게 말했다).
  //    상태 자체는 남겨 두었지만 채우지 않는다(아래 이펙트 참조).
  ...
  // 「내 업무」에서 넘어올 때 열 탭 (액션아이템 줄 → 'actions', 이슈 줄 → 'issues')
  const [editModalInitialTab, setEditModalInitialTab] = useState(null);
  ...
  const [isOwnerAuditOpen, setIsOwnerAuditOpen] = useState(false);   // 과제PL·작성자 계정 연결
  const [isMailRecipientOpen, setIsMailRecipientOpen] = useState(false); // 메일 수신처 뽑기
    ...
    const initAndLoadServerData = async () => {
      ...
      try {
        ...
        if (serverData && (serverData.projects?.length > 0 || serverData.performances?.length > 0)) {
          ...
          // ⚠️ 예전엔 여기서 `진행상태 === '완료'` 인 과제의 진행률을 100 으로 덮었다.
          //    **지웠다 (2026-08-11).** 진행률은 서버가 액션아이템에서 파생시키는 값이라
          //    (2026-08-03 규칙: 액션아이템이 정본, 진행상태는 진행률 계산에 관여하지 않는다)
          //    화면이 받아온 값을 다시 덮으면 정본과 어긋난다.
          //
          //    실제로 문제가 됐다 — 서버 값(33)을 화면이 100 으로 고쳐 저장하고 서버가
          //    다시 파생시키는 왕복이 `dt2_project_history` 에 `100 → 33 → 100` 으로
          //    남았다. 그 이력을 읽는 「멈춘 과제」 판정이 이 노이즈를 실제 변화로 본다.
          //
          //    액션아이템이 다 안 끝났는데 상태가 '완료' 인 것은 **고쳐야 할 데이터**이지
          //    화면이 숨겨 줄 일이 아니다. 서버가 그 모순을 400 으로 거절한다.
          ...
          // 고아 참조가 정리됐으면 서버에도 업로드 (백그라운드)
          // (진행률 보정 조건은 함께 지웠다 — 위 주석 참조)
          if (removedCount > 0) {
            if (removedCount > 0) {
  ...
  // ── 「내 일」 배지 (로그인/연도 변경 시) ──────────────────────────────
  //
  // **급한 카드 셋만** 세는 `summary` 를 부른다(기한 지난 항목·확인 대기·재검토 요청).
  // 전체를 만들면 서버가 진척 이력을 통째로 읽어 첫 화면 로딩에 그대로 얹힌다.
  //
  // ⚠️ 배지는 **미처리 건수**다. 열어봤다고 줄지 않는다 — 줄어들면 숫자가 뜻을 잃는다.
  // ⚠️ viewer 처럼 목록이 성립하지 않는 사람에게는 서버가 렌즈를 안 준다.
  //    그때는 `null` 로 두어 헤더가 버튼 자체를 안 그리게 한다.
  //
  // ⚠️ **화면을 스스로 열지 않는다** (2026-08-11 사용자 요청).
  //    처음에는 "급한 항목이 새로 생겼을 때만" 자동으로 열게 했는데, 들어올 때마다
  //    열렸다 — 항목의 키가 조금만 달라져도 '새것' 으로 잡혔기 때문이다
  //    (액션아이템은 늘 늘고 줄어서 키 집합이 거의 매번 바뀐다).
  //    가로막는 화면은 곧 아무도 안 보게 되므로, **알리기만 하고 데려가지는 않는다.**
  //    들어가는 길은 헤더의 종 버튼 하나다.
  useEffect(() => {
    if (!user) { setWorklistCount(null); return; }
    let alive = true;
    (async () => {
      try {
        const d = await fetchWorklist({ year: currentYear, summary: true });
        if (!alive) return;
        if (!d || !(d.lenses || []).length) { setWorklistCount(null); return; }
        setWorklistCount(d.urgentCount || 0);
      } catch {
        // 배지는 곁가지다. 실패해도 화면을 막지 않는다(조용히 안 그린다).
        if (alive) setWorklistCount(null);
      }
    })();
    return () => { alive = false; };
  }, [user, currentYear]);

  const dismissReportRejectPopup = () => setReportRejectPopup([]);
  // 「내 일」의 재검토 요청 줄에서 "보고서로" 를 누르면 여기로 온다.
  ...
  /**
   * 「내 업무」에서 인라인 저장이 끝난 뒤 **화면이 들고 있는 과제도 맞춘다.**
   *
   * 안 맞추면 다른 화면(간트·대시보드)이 옛 배열을 그대로 쓰고, 다음 저장 때
   * 그 옛 배열이 통째로 다시 올라가 **방금 한 처리가 되돌려진다.**
   * (배열 필드는 통째 교체라 특히 위험하다)
   */
  const handleWorklistProjectChanged = (uuid, patch) => {
    setProjects(prev => prev.map(p => ((p.uuid || p.id) === uuid ? { ...p, ...patch } : p)));
  };

  /**
   * 「내 일」에서 항목을 눌렀을 때 그 과제의 편집창을 연다.
   *
   * ⚠️ 서버는 uuid 만 준다(과제 본문을 다 실어 보내지 않는다). 화면이 이미 들고 있는
   *    목록에서 찾아 쓴다 — 못 찾으면 아직 안 받은 연도의 과제이므로 안내만 한다.
   */
  const openProjectFromWorklist = (uuid, tab = null) => {
    const found = (projects || []).find(p => (p.uuid || p.id) === uuid);
    if (!found) {
      showError('그 과제가 지금 화면에 없습니다. 연도를 바꾸면 보일 수 있습니다.');
      return;
    }
    // 어느 탭을 열지는 **부른 쪽이 안다** — 액션아이템 줄에서 왔으면 액션아이템 탭이다.
    // 기본 탭으로 열면 사용자가 방금 누른 줄을 화면에서 다시 찾아야 한다.
  ...
  const handleSubmitProject = async (newProjectData) => {
    try {
      // ⚠️ 진행률을 100 으로 덮던 줄을 지웠다 (2026-08-11). 서버가 액션아이템에서
      //    파생시키므로 화면이 정할 값이 아니다. 자세한 이유는 이 파일의 서버 데이터
      //    로드 부분(cleanupOrphanedPerformanceLinks 아래) 주석 참조.
  ...
  const handleUpdateProjectAndUpload = async (updatedProjectData) => {
    try {
      ...
      // ⚠️ 진행률을 100 으로 덮던 줄을 지웠다 (2026-08-11). 서버가 액션아이템에서
      //    파생시키므로 화면이 정할 값이 아니다. 이 경로는 서버로 저장하는 경로라
      //    특히 중요하다 — 여기서 덮으면 그 값이 이력에 그대로 남는다.
      ...
      // AI 폼 도우미가 채워 준 칸 이름. 같은 이유로 꺼내고 곧바로 지운다 —
      // 과제 데이터가 아니라 **변경 이력에 남길 표식**이다.
      const aiFilled = updatedProjectData.__dtAiFilled;
      delete updatedProjectData.__dtAiFilled;
      ...
      const saveResult = await saveProjectEdit({
        ...
        aiFilled   // AI 가 채운 칸. 어댑터가 이번 저장에 실린 것만 골라 보낸다.
      });
  ...
  const handleSubmitPerformanceAndUpload = async (performanceData) => {
    /*
      중복 이름 검사 — **try 밖에서, 예외로** 막는다 (2026-08-07).

      옛 동작은 try 안에서 `showError(); return;` 이었다. 두 가지가 틀렸다 —
        · 부르는 쪽(AddPerformanceModal)은 예외가 없으면 성공으로 보고
          "생성되고 서버에 업로드되었습니다" 를 띄웠다. 오류와 성공이 같이 떴다.
        · 과제 모달에서 부를 때는 저장도 안 된 성과를 그 과제의 성과목록에
          **자동 연결**해 버린다.
      try 안에서 던지면 `rethrowSaveError` 가 접두사를 한 겹 더 씌워
      "…오류: …오류: 동일한…" 이 되므로, 검사를 아예 앞으로 뺐다.
    */
    if (!performanceData.isEditing) {
      const isDuplicate = globalPerformances.some(perf =>
        perf.성과항목.toLowerCase().trim() === performanceData.성과항목.toLowerCase().trim() &&
      ...
      if (isDuplicate) throw new Error('동일한 성과 항목이 이미 존재합니다.');
    }
    ...
    try {
      ...
      if (performanceData.isEditing) {
        ...
        // 새로 생성 모드 (중복 검사는 이 함수 맨 위 — try 밖 — 으로 옮겼다, 2026-08-07)
        updatedPerformances = addPerformance(performanceData, globalPerformances);
        const addedPerformance = updatedPerformances[updatedPerformances.length - 1];
  ...
  /**
   * 성과 전체 삭제 — **서버에서 지운다.** admin 전용.
   *
   * 옛 동작은 로컬(IndexedDB/localStorage) 사본만 비우고 "삭제되었습니다" 라고
   * 말했다. 새로고침하면 서버에서 다시 내려와 되살아나는데도 그렇게 안내해서,
   * 연도별 일괄 삭제가 고친 것과 **같은 종류의 거짓말**이었다. (2026-08-02 변경)
   *
   * 로컬 반영은 서버가 성공한 **뒤에만** 한다 — 먼저 비우고 실패하면 화면만
   * 비어 있고 서버에는 남아 있는 상태가 된다.
   */
  const handleDeleteAllPerformances = async ({ expectedCount } = {}) => {
    const result = await saveBulkPerformanceDelete({
      expectedCount,
      reason: '성과 전체 삭제 (대시보드)',
    });

    // 서버가 모든 과제의 성과 연결을 끊었다. 화면도 같은 상태로 맞춘다.
    const updatedProjects = projects.map(project => ({ ...project, 성과목록: [] }));
    saveProjects(updatedProjects);
    setProjects(updatedProjects);
  ...
  const handleDeletePerformance = async (performanceId) => {
    try {
      // 'ALL' 은 더 이상 여기로 오지 않는다 — 모달이 `onDeleteAll`(서버 삭제)로
      // 간다. 옛 호출부가 남아 있어도 **로컬만 비우고 서버에 남는** 그 상태로
      // 돌아가지 않도록 막는다.
      if (performanceId === 'ALL') {
        console.warn('[DT] handleDeletePerformance("ALL") 은 더 이상 쓰지 않는다 — '
                     + 'handleDeleteAllPerformances 를 쓸 것');
        showError('전체 삭제는 성과 목록의 [전체 삭제] 버튼으로 실행하세요.');
        return;
      }
    ...
    performancesData.forEach((perfData) => {
      if (perfData.uuid) {
        ...
        const newPerformance = {
          ...
          소분류: perfData.소분류 || '',
          단위: perfData.단위 || '',
          현재수준: levelText(perfData.현재수준),
          목표수준: levelText(perfData.목표수준),
          월별실적여부: perfData.월별실적여부 || false,
          실적수준: levelText(perfData.실적수준),
          월별실적: 월별실적,
          설명: perfData.설명 || ''
        };
    ...
    try {
      ...
      performancesData.forEach((perfData) => {
        ...
        const performanceDataToSave = {
          ...
          목표수준: levelText(perfData.목표수준),
          월별실적여부: 월별실적여부,
          실적수준: 월별실적여부 ? '' : levelText(perfData.실적수준),
  ...
  /*
    지식 그래프 내보내기·서버 저장 핸들러를 내렸다 (2026-08-09).
    대시보드의 「관계도」 탭이 그 자리를 대신한다 — 저장하지 않고 열 때마다
    서버가 dt2_* 를 읽어 준다(`components/GraphView/`).
  */
  ...
  const handleCloseEditModal = () => {
    ...
    // 안 지우면 다음에 목록에서 그냥 연 과제도 액션아이템 탭으로 열린다.
  ...
  const renderContent = () => {
    ...
    // 과제·성과 추이 — 관계도·매트릭스와 같은 이유로 DashboardView 를 타지 않는다.
    // 서버에서 시계열을 직접 받고(화면 상태와 섞이면 숫자가 갈린다), 설정은
    // 단위 환산 규칙을 읽는 데만 쓴다.
    if (viewMode === 'dashboard' && dashboardSubTab === 'trendHistory') {
      return (
        <motion.div
          key="trendHistory"
    ...
    // 관계도 — 매트릭스와 같은 이유로 DashboardView 를 타지 않는다.
    // 자기 데이터를 서버에서 직접 받고(투영이라 화면 상태와 섞이면 안 된다),
    // 과제 목록은 노드를 눌렀을 때 상세 모달에 넘길 원본을 찾는 데만 쓴다.
    //
    // ★ 탭 버튼은 **admin 에게만** 보인다(Header). 다만 **API 는 안 막았다** —
    //   개발이 끝나면 어차피 열 것이라, 지금 403 을 걸었다가 나중에 푸는 것은
    //   일만 두 번이다. 그래서 여기도 권한으로 막지 않는다:
    //   버튼이 없으면 들어올 길이 없고, 들어온다 해도 보이는 것은 **자기가 볼 수
    //   있는 과제뿐**이다(`graph_view.build_graph` 가 `can_view_project` 로 거른다).
    if (viewMode === 'dashboard' && dashboardSubTab === 'graph') {
      return (
        <motion.div
          key="graph"
          initial={{ opacity: 0, y: 20 }}
        ...
        >
          {/*
            색표는 **다른 화면과 같은 것**을 넘긴다. 관계도가 자기 표를 따로 들면
            같은 사업부가 화면마다 다른 색이 된다.
          */}
          <GraphView
            projects={projects}
            performances={globalPerformances}
    ...
    //
    // 대시보드 서브탭이 아니라 `viewMode` 로 둔 이유: 서브탭 11개는 전부 전사 현황이고
    // 이건 나 개인의 작업대다. 게다가 서브탭은 대시보드 모드에서만 그려져서,
    // 기본 진입 화면(간트)에서 일하는 사람에게는 아예 안 보인다.
    if (viewMode === 'worklist') {
      return (
        <WorklistPage
          year={currentYear}
          projects={projects}
          onProjectsChanged={handleWorklistProjectChanged}
          onOpenProject={openProjectFromWorklist}
          onGoToReport={goToReportFromPopup}
      ...
      return (
        ...
        >
          <DashboardView
            ...
            /* 성과 휴지통에서 복구한 뒤 서버 데이터를 다시 받는다.
               복구는 서버에서만 일어나므로 이걸 안 부르면 화면이 옛 목록 그대로다. */
            onPerformanceRestored={() => executeServerDownload({ silent: true, preserveMetadata: true })}
          />
        </motion.div>
      );
    } else {
  ...
  return (
    <Container>
      <Header
        ...
        onAuditOwnerLinks={() => setIsOwnerAuditOpen(true)}
        onMailRecipients={() => setIsMailRecipientOpen(true)}
        onGenerateReport={() => setIsReportModalOpen(true)}
        worklistCount={worklistCount}
        onOpenWorklist={() => setViewMode('worklist')}
        onGeneratePdfReport={() => setIsPdfReportModalOpen(true)}
      />

      {/*
        「내 일」 드로어 (2026-08-11).
        헤더의 종 버튼으로 열고, 급한 항목이 **새로 생겼을 때만** 스스로 열린다.
        여기서 판정·정렬·미룸을 전부 서버가 끝내서 주므로 화면은 그리기만 한다.
      */}

      {/* 재검토 요청 알림 팝업 — 「내 일」에 흡수됐다. 아무도 채우지 않으므로 안 뜬다.
          되살리려면 서버 판정(worklist 의 reportReject 카드)을 쓸 것. 이름 매칭으로
          되돌리면 서버와 다시 갈린다. */}
      ...
      <AddProjectModal
        ...
        // 과제 모달 안에서 고친 성과도 **서버에 저장**한다 (2026-08-07).
        // 안 넘기면 AddPerformanceModal 이 로컬 전용 경로로 떨어져, 고친 내용이
        // 모달을 닫는 순간 사라진다. 성과 화면(아래 AddPerformanceModal)은
        // 원래부터 넘기고 있어서 거기서만 저장이 됐다.
        onSubmitPerformanceAndUpload={handleSubmitPerformanceAndUpload}
      />
      ...
      <AddPerformanceModal
        ...
        // 대시보드에서만 넘긴다 — 과제 추가·수정 모달에서 '전체 삭제'는
        // "그 과제의 연결을 비운다" 는 뜻이라 서버를 지우면 안 된다.
        onDeleteAll={handleDeleteAllPerformances}
        settingsData={currentSettingsData}
        globalPerformances={globalPerformances}
        showSuccess={showSuccess}
        showError={showError}
      ...
      <EditProjectModal
        ...
        globalPerformances={globalPerformances}
        onSubmitPerformance={handleSubmitPerformance}
        // 과제 편집 → 과제성과 → 연필 버튼으로 고친 성과도 **서버에 저장**한다
        // (2026-08-07 버그 수정). 이게 없어서 로컬 전용 경로로 떨어졌고,
        // "수정되었습니다" 라고 말하면서 서버에는 아무것도 안 갔다.
        onSubmitPerformanceAndUpload={handleSubmitPerformanceAndUpload}
      ...
      */}
      ...
      {/*
        ...
        참여인력과 **다른 칸**이라 위 화면으로는 못 고친다. 적용 후 서버 데이터를 다시
        받는 이유도 같다 — `pl_knox_id` 만 서버에서 바뀌고 화면의 `projects` 는 낡는다.
      */}
      {/* 메일 수신처 — 서버를 부르지 않는다. 필요한 knoxId 가 이미 과제 목록에 있다. */}
      <MailRecipientModal
        open={isMailRecipientOpen}
        onClose={() => setIsMailRecipientOpen(false)}
        projects={projects}
        divisionColors={DIVISION_COLORS}
        currentYear={currentYear}
      />
      ...
      <OwnerLinkAuditModal
        isOpen={isOwnerAuditOpen}
        onClose={() => setIsOwnerAuditOpen(false)}
        onApplied={() => executeServerDownload({ silent: true })}
        showSuccess={showSuccess}
        showError={showError}
      />
      ...
      {/* AI 에이전트 (도구로 과제를 조회·수정) — **로그인한 사람 누구나**(2026-08-08).
          쓰기는 관리자만이고 **서버가 강제한다**(`/api/dt-v2/ai/agent` 가 비관리자의
          요청을 읽기로 되돌린다). 화면은 그 사실을 표시만 한다 —
          숨기면 "이런 기능이 있는 줄도 모르는" 상태가 되는데, 조회는 누구나 해도 되는 일이다. */}
      <AiAgentPanel isAdmin={Boolean(user?.role === 'admin' || user?.is_admin)} />
    </Container>
  );
};
