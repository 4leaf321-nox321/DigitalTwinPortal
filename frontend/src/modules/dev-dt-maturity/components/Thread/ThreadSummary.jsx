...
//   ThreadOverviewRows — 전체 요약 표의 아래 줄: 스레드마다 사업부별 연속성 % · 도달 단계 (최약 표시는 뺐다 2026-08-29)
...
const Small = styled.div.attrs({ className: 'ov-note' })`font-size: 0.75rem; color: #64748b; margin-top: 0.2rem; white-space: normal; overflow-wrap: anywhere;`;   // 요약의 코멘트 토글이 이 반도 숨긴다
...
export const ThreadOverviewRows = ({ boards }) => {
  ...
  return (
    <>
      <tr><SecHead colSpan={boards.length + 2}><strong>스레드</strong>구간을 모아 줄로 — 연속성(자동 전달 이상 %) · 도달 단계 · 수집률 · 활용률</SecHead></tr>
      {threads.map(t => (
        <tr key={t.thread_key}>
