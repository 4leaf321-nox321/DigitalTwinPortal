...
const Card = styled.div`
  border: 1px solid #e2e8f0; border-left: 3px solid #f59e0b; border-radius: 0.5rem; padding: 0.7rem 0.85rem;
...
const ProposalModal = ({ divisionId, axesBySector = {}, onClose, onChanged }) => {
  ...
  const load = async () => {
    try {
      const r = await maturityApi.listProposals(divisionId);
      setRows(r.data || []); setError(null);
