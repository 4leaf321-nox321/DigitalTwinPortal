...
const Tiles = styled.div`display: grid; grid-template-columns: repeat(auto-fill, minmax(11rem, 1fr)); gap: 0.45rem;`;
const Tile = styled.button`
  position: relative; text-align: left; border-radius: 0.5rem; padding: 0.5rem 0.6rem; min-height: 3.6rem; font-family: inherit; cursor: pointer;
...
`;
const TName = styled.div`font-size: 0.8125rem; font-weight: 700; line-height: 1.25; overflow: hidden; display: -webkit-box; -webkit-line-clamp: 2; -webkit-box-orient: vertical;`;
...
const TileBoard = ({ subjects = [], axes = [], onOpenPair, allMode = false }) => {
  ...
  const groups = useMemo(() => {
    ...
    subjects.forEach(s => (s.pairs || []).forEach(p => {
      const dept = p.agent?.department_name || '부서 미지정';
            ...
            <Group key={dept} aria-label={dept}>
              ...
              <Tiles>
                {rows.map(({ p, subject }) => {
                  ...
                  return (
                    <Tile key={p.id} type="button" $c={c} $dark={isDark(c)} $empty={v.idx == null} $stale={v.stale}
                          title={`${subject.name} × ${p.agent?.name || ''} — ${axis.label}: ${label}${v.stale ? ' · 재평가 필요' : ''}`}
                          onClick={() => onOpenPair && onOpenPair(p.id)}>
                      <TName>{p.agent?.name || subject.name}</TName>
                      <TSub>{subject.name}</TSub>
