...
const BulkEdgeModal = ({ 
  ...
  const processPasteData = (text) => {
    ...
    if (parsedData.length === 0) {
      if (showError) {
        showError(`유효한 엣지 데이터를 찾을 수 없습니다.
...
박영희	AI회사	근무	정규직`, '데이터 형식 오류');
    ...
    if (showSuccess) {
      showSuccess(`${parsedData.length}개의 엣지 데이터를 성공적으로 추가했습니다!`, '데이터 추가 완료');
    } else {
...
2. 먼저 필요한 노드들을 생성해주세요`, '유효한 엣지 없음');
  ...
  useEffect(() => {
    const handleKeyDown = (e) => {
      // 붙여넣기 모달이 열려있으면 textarea의 기본 붙여넣기를 방해하지 않음
      if (isOpen && !showPasteModal && e.ctrlKey && e.key === 'v') {
        e.preventDefault();
  ...
  }, [isOpen, showNodePicker, showPasteModal]);
        ...
        <div className="paste-modal-overlay" onClick={() => setShowPasteModal(false)}>
          <div className="paste-modal-content" onClick={e => e.stopPropagation()}>
            <div className="paste-modal-header">
              <h3>데이터 붙여넣기</h3>
              <button className="modal-close-btn" onClick={() => setShowPasteModal(false)}>
                <X size={20} />
              </button>
            </div>
            <div className="paste-modal-body">
              <p className="paste-instruction">
                아래 입력창에 엑셀이나 텍스트 데이터를 <strong>Ctrl+V</strong>로 붙여넣어주세요.
              </p>
              <textarea
                className="paste-textarea"
                value={pasteText}
                onChange={(e) => setPasteText(e.target.value)}
                placeholder="여기에 데이터를 붙여넣으세요 (Ctrl+V)&#10;&#10;예시:&#10;김철수	박영희	협업&#10;박영희	AI회사	근무	정규직&#10;AI회사	데이터분석	참여"
                rows={10}
                autoFocus
              />
              <div className="paste-format-guide">
                <strong>지원 형식:</strong> 탭, 쉼표, 세미콜론, 파이프로 구분된 데이터
              </div>
            </div>
            <div className="paste-modal-footer">
              ...
              <button className="btn btn-primary" onClick={handleManualPaste}>
                데이터 추가
              </button>
            </div>
          </div>
