    ...
    <div className="graph-controls">
      ...
      <div className="control-group">
        <button
          ...
          title="물리 시뮬레이션 토글 (단축키: Spacebar)"
      ...
      {/* 노드 간격 조절 슬라이더 */}
      <div className="control-group spacing-control">
        <label className="spacing-label" title="노드 간격 조절">
          <span className="spacing-text">간격</span>
          <input
            type="range"
            min="50"
            max="500"
            value={nodeSpacing}
            onChange={(e) => onNodeSpacingChange && onNodeSpacingChange(Number(e.target.value))}
            className="spacing-slider"
          />
          <span className="spacing-value">{nodeSpacing}</span>
        </label>
      </div>
