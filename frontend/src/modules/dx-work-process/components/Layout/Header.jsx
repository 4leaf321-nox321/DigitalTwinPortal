...
const Header = ({
  ...
  const [showServerSync, setShowServerSync] = useState(false);
  ...
  // 우측 액션 버튼들 (샘플, Import/Export, 서버 저장/불러오기, 설정)
  const rightContent = (
    <div className="header-actions">
      {/* 샘플 데이터 로드 */}
      ...
      <button
        className="header-btn action-btn"
        onClick={() => setShowServerSync(true)}
        title="서버 저장/불러오기"
        style={{
  ...
  return (
    <>
      ...
      {/* 서버 저장/불러오기 모달 */}
      <ServerSyncModal
        isOpen={showServerSync}
        onClose={() => setShowServerSync(false)}
        currentGraphData={currentGraphData}
      ...
      {/* 헤더 스타일 */}
      <style>{`
        /* Header Center Content - 가로 배치 */
        .knowledge-graph-header .header-center-content {
        ...
        /* Header Actions - 가로 배치 */
        .knowledge-graph-header .header-actions {
          display: flex !important;
          flex-direction: row !important;
          align-items: center !important;
          gap: 0.5rem !important;
        ...
        /* Header Right - 가로 배치 */
        .knowledge-graph-header .header-right {
          display: flex !important;
          flex-direction: row !important;
          align-items: center !important;
        ...
        @media (max-width: 1400px) {
          .knowledge-graph-header .header-btn span,
          .knowledge-graph-header .action-btn span {
            display: none;
          }

          .knowledge-graph-header .header-btn,
          .knowledge-graph-header .action-btn {
          ...
          .knowledge-graph-header .header-divider {
            display: none;
          }
        }
