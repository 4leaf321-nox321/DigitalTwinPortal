...
const NodeTypesSection = ({
  ...
  const [newNodeTypeColor, setNewNodeTypeColor] = useState('#4CAF50');

  // 다음 기본 색상 가져오기
  const getNextDefaultColor = () => {
    const usedColors = nodeTypes.map(t => t.color).filter(Boolean);
    const availableColor = DEFAULT_NODE_COLORS.find(c => !usedColors.includes(c));
  ...
  const handleAddNodeType = () => {
    ...
    const newType = {
      ...
      color: newNodeTypeColor || getNextDefaultColor()
    };

    setNodeTypes(prev => [...prev, newType]);
    setNewNodeType('');
    setNewNodeTypeColor(getNextDefaultColor());
    ...
    <div className="type-section">
      ...
      <div className="section-actions">
        <div className="add-type-form">
          ...
          <input
            type="color"
            value={newNodeTypeColor}
            onChange={(e) => setNewNodeTypeColor(e.target.value)}
