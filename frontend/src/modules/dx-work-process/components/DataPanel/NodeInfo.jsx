...
const NodeInfo = ({ 
  ...
  // 현재 노드의 타입이 목록에 없으면 추가
  const currentNodeType = isEditing ? editedNode.type : node.type;
  const typeExists = availableNodeTypes.some(t => t.id === currentNodeType);
  if (!typeExists && currentNodeType) {
    availableNodeTypes.push({
      id: currentNodeType,
      label: getNodeTypeDisplayName(currentNodeType)
    });
  }
