...
const EditNodeModal = ({ isOpen, onClose, onSave, node }) => {
  ...
  useEffect(() => {
    if (node && isOpen) {
      ...
      setHeaderOnly(node.data?.headerOnly || false);
      setTextAlign(node.data?.textAlign || 'left');
      setHeaderTextColor(node.data?.headerTextColor || 'white');
      setFontSize(node.data?.fontSize || '0.8rem');
    }
  }, [node, isOpen]);
