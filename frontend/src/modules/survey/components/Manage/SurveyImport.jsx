...
const COLUMN_GUIDE = [
  ...
  { name: '보기', desc: '| 로 구분. 순서가 있으면 «보기=점수»(1~5)' },
...
// 머리글 행, 빈 역할(=전원), 섹션 이어받기, 쉼표로 여럿 적은 프로세스,
// `|` 로 나눈 보기, 유형 다섯 가지, 필수/선택, 연결키.
//
// ⚠️ **역할·프로세스 이름을 여기 박아 두지 않는다.**
//    한 번 그렇게 했다가 실제로 낡았다 — 역할이 '과제 멤버'에서 '과제 참여인력'
//    으로 바뀌자 예시 채우기가 **곧바로 400 을 내는 표**를 넣어 주었다. 형식을
//    배우려고 누르는 버튼이 첫인상으로 오류를 주는 셈이다.
//    그래서 서버가 준 목록(GET /manage/options)으로 그때그때 만든다. 목록이
//    바뀌면 예시도 따라 바뀐다.
function buildExample(roles = [], processes = []) {
  const header = ['섹션', '역할', '프로세스', '문항', '유형', '보기', '필수', '도움말', '연결키'];
  // 목록에서 자리를 잡아 쓴다. 짧으면 있는 것으로 돌려 쓰고, 아예 없으면
  // 그 칸을 비운다(빈 칸은 '전원'이라 언제나 유효하다).
  const role = (i) => roles[i] || roles[roles.length - 1] || '';
  const proc = (i) => processes[i] || processes[processes.length - 1] || '';
  const twoProc = processes.length >= 2
    ? `${processes[0]},${processes[1]}` : proc(0);

  return [
    header,
    ['공통', '', '', '올해 우리 조직의 AX 목표를 알고 있습니까?', '척도', '', '예',
     '1=전혀 모른다, 5=명확히 안다', 'organization:readiness'],
    ['공통', '', '', 'AX 도구를 실제 업무에서 얼마나 쓰십니까?', '객관식',
     '매일|주 1~2회|가끔|쓰지 않음', '예', '', ''],
    // 섹션을 비워 직전 행을 잇는다(엑셀 병합처럼 쓰는 자리).
    ['과제 수행', role(3), twoProc, '담당 과제에 필요한 데이터가 제때 모입니까?',
     '척도', '', '예', '데이터가 없어 과제가 멈춘 적이 있는지 떠올려 보세요',
     'organization:redesign'],
    ['', role(3), '', '과제를 막는 것을 모두 고르세요', '복수선택',
     '데이터 부족|도구 미숙|시간 부족|타 부서 협조', '아니오', '', ''],
    ['과제 관리', role(2), '', '과제원의 역할과 책임이 분명합니까?', '척도', '', '예',
     '', 'organization:role'],
    ['', role(2), proc(2), '연계가 필요한 부서와 협의가 원활합니까?', '척도', '',
     '아니오', '', ''],
    ['운영', role(1), '', '먼저 풀어야 할 것을 순서대로 놓으세요', '순위',
     '예산|인력|교육|표준화', '예', '', ''],
    ['운영', role(0), '', '내년 AX 투자 계획은 어떻습니까?', '객관식',
     '확대|유지|축소|미정', '예', '', 'organization:return'],
...
const toText = (rows) => rows.map(r => r.join(TAB)).join(NEWLINE);
...
const roleCounts = useMemo(() => countAxis(rows, 'roles'), [rows]);
  ...
  const fillExample = () => {
    ...
    setText(toText(buildExample(axisOptions.roles, axisOptions.processes)));
  };
  ...
  return (
    <Wrap>
      ...
      <Guide>
        ...
        {/* 순서가 있는 객관식을 진단 레벨로 쓰는 길. 이걸 모르면 '얼마나 자주'
            같은 문항이 진단에 아무 영향을 못 주고, 왜 안 들어가는지도 모른다. */}
        <GuideNote>
          <strong>순서가 있는 보기</strong>에는 점수를 붙일 수 있습니다 —{' '}
          <code>매일=5|주 1~2회=4|가끔=2|쓰지 않음=1</code>. 점수를 붙이고 연결키를
          적으면 그 문항이 <strong>척도 문항과 같은 자격으로 진단 레벨</strong>에
          들어갑니다. 전부 붙이거나 전부 안 붙이거나이고, 객관식(단일 선택)에만
          됩니다. <strong>응답자에게는 점수가 보이지 않습니다.</strong>{' '}
          &lsquo;데이터 정합성&rsquo;처럼 <strong>순서가 없는 보기에는 붙이지
          마세요</strong> — 어느 쪽이 높은 단계인지 정해질 수 없습니다.
        </GuideNote>
      </Guide>
