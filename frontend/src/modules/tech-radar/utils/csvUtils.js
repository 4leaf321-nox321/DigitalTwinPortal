/**
 * Tech Radar 데이터의 CSV 변환을 위한 유틸리티
 */

/**
 * Tech Radar 데이터를 CSV 형태로 변환
 * @param {Array} technologies - 기술 배열
...
export const csvToTechnologies = (csvData) => {
  ...
  return csvData.map((row, index) => {
    const id = row.ID || row.id || `tech_${Date.now()}_${index}`;
    ...
    return {
      id,
      name: row.Name || row.name || `Technology ${index + 1}`,
...
export const validateTechRadarCSVData = (data) => {
  ...
  if (!hasNameField) {
    result.errors.push('기술 이름 필드 (Name 또는 name)가 필요합니다.');
    result.isValid = false;
  }

  // 유효한 섹터/사분면과 링 값들 (현재 샘플 데이터에 맞춰 수정)
  const validSectors = [
    'simulation-methods', 'digital-twin-platforms', 'ai-ml-simulation', 'visualization-collaboration', // 샘플 데이터 기준
  ...
  if (duplicateNames.length > 0) {
    result.warnings.push(`중복된 기술명이 있습니다: ${[...new Set(duplicateNames)].join(', ')}`);
  }
...
/**
 * Tech Radar 데이터를 섹터별로 그룹화하여 CSV로 변환
 * @param {Array} technologies - 기술 배열
...
export const technologiesToStatisticsCSV = (technologies) => {
  ...
  return Object.entries(stats).map(([ring, data]) => ({
    ...
    'Total Technologies': data.total,
    'New Technologies': data.new,
    'Moved Technologies': data.moved,
