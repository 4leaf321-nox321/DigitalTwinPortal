...
const BulkAddTechnologyModal = ({ 
  ...
  const addRow = () => {
    setTechnologies([...technologies, { name: '', sector: '', ring: '', description: '' }]);
  };
  ...
  const parseClipboardData = (clipboardText) => {
    ...
    if (lines.length === 0) {
      ...
      return [{ name: '', sector: '', ring: '', description: '' }];
    }
    ...
    if (result.length === 0 || result.every(tech => !tech.name.trim())) {
      ...
      result.push({ name: '', sector: '', ring: '', description: '' });
    }
  ...
  const validateForm = () => {
    ...
    if (filledTechnologies.length === 0) {
      newValidationErrors.push('최소 하나의 기술을 입력해주세요.');
    ...
    filledTechnologies.forEach((tech, index) => {
      ...
      if (!tech.name.trim()) {
        newErrors[`${originalIndex}-name`] = '기술 이름은 필수입니다.';
        newValidationErrors.push(`${originalIndex + 1}번째 행: 기술 이름이 비어있습니다.`);
      }
      ...
      if (duplicates.length > 1) {
        newErrors[`${originalIndex}-name`] = '중복된 기술명입니다.';
        newValidationErrors.push(`${originalIndex + 1}번째 행: "${tech.name}" 중복된 기술명입니다.`);
      }
    });
  ...
  const handleClose = () => {
    setTechnologies([{ name: '', sector: '', ring: '', description: '' }]);
    setErrors({});
    setValidationErrors([]);
    setPasteNotification('');
    onClose();
  ...
  const exportTemplate = () => {
    ...
    data.sectors?.forEach(sector => {
      ...
      data.rings?.forEach(ring => {
        sampleData.push([
          ...
          sector.name,
          ring.name,
          `${sector.name} 분야의 ${ring.name} 단계 기술`
        ]);
        sampleCounter++;
      });
    });
    ...
    if (sampleData.length === 0) {
      sampleData.push(
        ['sample1', 'Frontend', 'Adopt', 'Frontend 분야의 Adopt 단계 기술'],
        ['sample2', 'Backend', 'Trial', 'Backend 분야의 Trial 단계 기술'],
        ['sample3', 'Infrastructure', 'Assess', 'Infrastructure 분야의 Assess 단계 기술'],
        ['sample4', 'Data', 'Hold', 'Data 분야의 Hold 단계 기술']
      );
    }
      ...
      <form onSubmit={handleSubmit} className="bulk-technology-form">
        ...
        <div className="bulk-form-guide">
          <div className="guide-content">
            <Table size={18} />
            <div>
              <p><strong>여러 기술을 한 번에 추가하세요</strong></p>
          ...
          <table className="technology-table">
            <thead>
              <tr>
                ...
                <th className="col-name">기술 이름 *</th>
        ...
        <div className="table-controls">
          ...
          <span className="table-info">
            {technologies.length}개 행 | 필수 항목: 기술 이름, 섹터, 성숙도 | <kbd>Ctrl+V</kbd>로 붙여넣기
          </span>
        </div>
        ...
        <div className="form-actions">
          ...
          >
            <Plus size={16} />
            {technologies.filter(tech => tech.name.trim()).length}개 기술 추가
          </button>
        </div>
      </form>
    </Modal>
