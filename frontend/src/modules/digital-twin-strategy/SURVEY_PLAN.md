...
포탈의 `role` 에는 과제멤버·PL·사무국장 구분이 **없습니다.** `users.department`
도 비어 있고, PL 은 과제 데이터로 유도할 수 있지만 과제 멤버는 알 방법이
없습니다. 그래서 **사업부와 같은 방식**으로 갑니다 — 유도되면 기본값, 안 되면
직접 고르고, **고른 값을 응답에 저장해 집계 축으로** 씁니다.

```
survey.roles / survey.processes          이 설문이 물을 선택지 (비면 안 묻는다)
question.audience_roles / _processes     이 문항을 볼 사람 (**빈 배열이면 전원**)
response.respondent_role / _process      응답자가 고른 값 → 집계 축
```

⚠️ **빈 배열 = 전원**입니다. NULL 과 다르게 다루면 안 됩니다. "아무도 못 본다"가
기본이 되면 임포트 한 번 잘못해서 **아무도 답할 수 없는 설문**이 만들어지고,
그걸 눈치채는 데 며칠 걸립니다.
