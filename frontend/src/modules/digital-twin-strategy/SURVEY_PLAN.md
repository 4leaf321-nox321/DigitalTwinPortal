...
문항의 `link_key` 는 저장되지만 **읽는 코드가 아직 없습니다.** 어떻게 이을지는
[LINK_PLAN.md](./LINK_PLAN.md) 에 따로 적었습니다 — 요점은 설문이 **근거지
결론이 아니라는** 것입니다. 집계는 제안값으로 뜨고, 진단값은 사람이 반영을
눌러야 바뀝니다.

그 문서의 0단계(연결키를 목록에서 고르게 하기)는 **설문을 운영에 돌리기 전에**
해야 합니다. 지금은 `organization:readiness` 를 손으로 타이핑하는데, 오타가
나도 조용히 안 붙고 그 사실은 진단을 만들 때가 되어서야 드러납니다.
