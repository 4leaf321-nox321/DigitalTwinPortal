...
import { MEASUREMENT_COLUMNS, createEmptyMeasurementRow } from '../../utils/traFileParser';
...
const MeasurementDataTable = ({ data = [], onChange, readOnly = false }) => {
  const [editingId, setEditingId] = useState(null);
  const [editValues, setEditValues] = useState({});

  const handleAdd = () => {
    const newRow = createEmptyMeasurementRow();
  ...
  const handleCancelEdit = () => {
    ...
    if (originalRow &&
        originalRow.extensometer === 0 &&
        originalRow.loadCell === 0 &&
        originalRow.specimenWidth === 0) {
            ...
            <Thead>
              <tr>
                ...
                {MEASUREMENT_COLUMNS.map(col => (
                  <Th key={col.key}>{col.label} ({col.unit})</Th>
                ))}
                {!readOnly && <Th>작업</Th>}
              </tr>
            </Thead>
                    ...
                    <Td key={col.key}>
                      {editingId === row.id && !readOnly ? (
                        <Input
                          ...
                          autoFocus={col.key === 'extensometer'}
                        />
                      ) : (
