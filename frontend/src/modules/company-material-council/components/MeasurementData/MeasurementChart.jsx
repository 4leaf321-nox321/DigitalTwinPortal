...
const AXIS_OPTIONS = [
  { key: 'extensometer', label: 'Extensometer (mm)' },
  { key: 'loadCell', label: 'Load Cell (N)' },
  { key: 'specimenWidth', label: 'Specimen Width (mm)' },
];

const MeasurementChart = ({ data = [] }) => {
  const svgRef = useRef(null);
  const containerRef = useRef(null);
  const [xAxisKey, setXAxisKey] = useState('extensometer');
  const [yAxisKey, setYAxisKey] = useState('loadCell');
  ...
  useEffect(() => {
    ...
    // 브러시 (드래그로 영역 선택) - 포인트보다 먼저 그려서 아래에 배치
    const brush = d3.brush()
      ...
      .on('end', (event) => {
        ...
        const newYDomain = [yScale.invert(y1), yScale.invert(y0)]; // y는 반전
    ...
    // 포인트 그리기 - 브러시 위에 그려서 마우스 이벤트 받을 수 있게
    ...
    dotsGroup.selectAll('.dot')
      ...
      .on('mouseover', function(event, d) {
        ...
        d3.select(this).attr('r', 6).style('fill', '#7c3aed');
      ...
      .on('mouseout', function() {
        d3.select(this).attr('r', 4).style('fill', '#8b5cf6');
        setPointTooltip({ visible: false, x: 0, y: 0, xValue: 0, yValue: 0 });
      });

    // Ctrl+클릭으로 확대, Shift+클릭으로 축소, 일반 클릭으로 리셋
    svg.on('click', (event) => {
      // 브러시 영역 내에서 클릭한 경우 무시
