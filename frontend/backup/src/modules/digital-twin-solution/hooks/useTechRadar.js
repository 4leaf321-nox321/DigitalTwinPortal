...
// 고유한 ID 생성 함수
const generateUniqueId = (existingTechnologies = [], prefix = 'solution') => {
  let id;
  do {
...
export const useTechRadar = () => {
  ...
  // 솔루션 추가 - 고유한 ID 보장 및 isAdopted 기본값 설정
  ...
  const exportData = () => {
    ...
    link.href = url;
    link.download = `digital-twin-solution-map-${new Date().toISOString().split('T')[0]}.json`;
      ...
      reader.onload = (e) => {
        try {
          ...
          if (data.data && data.data.sectors && data.data.rings && data.data.technologies) {
            ...
            reject(new Error('올바른 디지털 트윈 솔루션 맵 데이터 형식이 아닙니다.'));
            return;
          }

          // 가져온 솔루션들의 ID 고유성 보장 및 isAdopted 기본값 설정
          ...
          setSelectedTechnology(null);
          resolve(`${processedTechnologies.length}개의 솔루션을 성공적으로 불러왔습니다.`);
