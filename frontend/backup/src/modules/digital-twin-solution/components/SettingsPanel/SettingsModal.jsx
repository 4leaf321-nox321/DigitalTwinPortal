...
const SettingsModal = ({ 
  ...
  useEffect(() => {
    ...
    const hasChanged = (
      JSON.stringify(localSettings.sectors) !== JSON.stringify(data.sectors || []) ||
      JSON.stringify(localSettings.rings) !== JSON.stringify(data.rings || []) ||
      JSON.stringify(localSettings.categories) !== JSON.stringify(data.categories || []) ||
      JSON.stringify(localSettings.subcategories) !== JSON.stringify(data.subcategories || []) ||
      JSON.stringify(localSettings.objectives) !== JSON.stringify(data.objectives || [])
    );
    
    setHasChanges(hasChanged);
  }, [localSettings, data, isOpen]);

  const tabs = [
    { id: 'sectors', label: '사업부', key: 'sectors' },
    { id: 'rings', label: '진행상태', key: 'rings' },
    { id: 'categories', label: '대분류', key: 'categories' },
    { id: 'subcategories', label: '소분류', key: 'subcategories' },
    { id: 'objectives', label: '과제목표', key: 'objectives' }
  ];
    ...
    switch (category) {
      ...
      case 'rings':
        return {
          ...baseItem,
          color: '#64748B',
          radius: 100 + (localSettings[category].length * 50),
          description: ''
        };
      default:
        return baseItem;
  ...
  const handleReset = () => {
    if (window.confirm('변경사항을 모두 되돌리시겠습니까?')) {
      setLocalSettings({
        sectors: [...(data.sectors || [])],
        rings: [...(data.rings || [])],
        categories: [...(data.categories || [])],
      ...
      >
        <div className="item-content">
          <div className="item-main">
            ...
            {(category === 'sectors' || category === 'rings') && (
          ...
          {category === 'rings' && (
            <div className="item-radius">
              ...
              <input
                type="number"
                value={item.radius || 100}
                onChange={(e) => handleUpdateItem(category, item.id, { radius: parseInt(e.target.value) })}
                min="50"
                max="500"
