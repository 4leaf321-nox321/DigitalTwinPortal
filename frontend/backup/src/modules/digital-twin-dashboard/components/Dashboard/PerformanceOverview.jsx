...
const Container = styled(motion.div)`
  ...
  min-height: 750px; /* 고정 높이에서 최소 높이로 변경 */
  display: flex;
...
const TreemapSection = styled.div`
  ...
  gap: 1rem;
  /* 좌측 섹션의 최대 높이를 제한하여 레이아웃 안정성 확보 */
  max-height: 600px;
`;
...
const TreemapContainer = styled.div`
  ...
  min-width: 0;
  height: 450px;
  max-height: 450px; /* 트리맵은 고정 높이 유지 */
  border: 1px solid #e2e8f0;
  border-radius: 0.5rem;
  overflow: hidden;
  position: relative;
...
const PerformanceOverview = ({ projects, divisionColors, statusColors }) => {
  ...
  const renderTreemap = useCallback(() => {
    ...
    // 클릭 이벤트 - 최신 상태를 참조하기 위해 콜백 함수로 처리
  ...
  }, [flatTreemapData, selectedFilter]);
