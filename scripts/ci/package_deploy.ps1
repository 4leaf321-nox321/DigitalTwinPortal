...
if (Test-Path .\\mcp_server) {
  ...
  # Bundle the MCP dependencies into their own site-packages, kept apart from
  # the backend's. mcp pulls in starlette, pydantic, uvicorn and anyio, which
  # the backend has no reason to carry -- see mcp_server/README.md, "왜 별도
  # 폴더·별도 venv 인가". The production server has no internet, so shipping
  # them is what makes the MCP server runnable there at all.
  $mcpReq = '.\\deploy\\mcp_server\\requirements.txt'
  if (Test-Path $mcpReq) {
    $mcpSitePackages = Join-Path .\\deploy 'mcp_server\\site-packages'
    Write-Host 'Installing mcp_server dependencies into deploy/mcp_server/site-packages'
    python -m pip install -r $mcpReq -t $mcpSitePackages
    if ($LASTEXITCODE -ne 0) { Write-Error "mcp_server pip install failed (exit $LASTEXITCODE)"; exit 1 }

    foreach ($mod in @('mcp', 'httpx', 'anyio', 'pydantic', 'starlette')) {
      if (-not (Get-ChildItem -Path $mcpSitePackages -Filter $mod -Force -ErrorAction SilentlyContinue)) {
        Write-Error "mcp_server/site-packages is missing '$mod' - the MCP server would not run."
        exit 1
      }
    }
    Write-Host 'mcp_server dependency check passed'
  } else {
    Write-Host 'No mcp_server/requirements.txt; skipping mcp dependency packaging.'
  }
}
...
if (Test-Path .\\deploy\\mcp_server) {
  Write-Host 'Adding run_mcp_server.ps1 template'
