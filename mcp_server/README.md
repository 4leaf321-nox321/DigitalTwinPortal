...
| `maturity_assess` | **한 축을 매긴다** — 근거 필수. rung·value·flags(묶음·바탕) |
| `maturity_set_defect` | 모델링 수준의 **불량 유형 표** 칸 하나(시험·시장 × 연-월) |
| `maturity_reached` | 그 칸에 **언제 올라왔나** — 옛 자료를 그 달로 |
| `maturity_bulk` | 표를 붙여넣어 여럿을 한 번에(먼저 dry_run) |
| `maturity_bulk_kinds` | 그 표에 어떤 열·어떤 값이 필요한가 |
| `maturity_list_records` | **건 기록** 목록 — review(해석 활용) · thread_case(연계 개발) |
| `maturity_record_stats` | 그 기록의 연간 셈 — 비율·리드타임·**정착 후보** |
| `maturity_add_record` | 건 한 줄 적기 |
| `maturity_update_record` | 건 한 줄 고치기 |
| `maturity_promote_review` | 정착 후보 → **상시 시험 항목**(척도로 넘어간다) |
| `maturity_add_system` | 시스템 사전에 더하기(전사 하나 — 같은 이름은 거절) |
| `maturity_update_system` | 시스템 한 줄 고치기(연계 수단·상태) |
| `maturity_add_org` | 조직 사전에 더하기(포탈 부서는 저절로 들어온다) |
| `maturity_name_catalog` | 「사용 툴」·「제품군」의 **표준 이름 목록** — 적기 전에 |
| `maturity_name_audit` | 표준 밖 이름과 고칠 후보 — **넣은 뒤에 한 번** |
| `maturity_rename` | 사업부 전체에서 이름 하나를 표준으로 |
| `maturity_add_segment` | **디지털 스레드의 구간**을 만든다(대상·연계·속성이 한 번에) |
| `maturity_threads` | 표준 스레드와 구간 정의 — 구간을 세울 재료 |
| `maturity_thread_dicts` | 스레드의 시스템·조직 사전 — 출발·매개·도착에 넣을 id |

### 도구가 61개인데 안 헷갈리나

이름(`maturity_`)만으로는 목록에서 첫 줄만 보이는 클라이언트에 약해서, **설명 첫 줄도
`[성숙도]` 로 시작**하게 했다. 반대로 대시보드의 길잡이 둘(`describe_fields`·
`describe_data`)에는 「성숙도는 여기 없다 — `maturity_describe` 를 쓰라」를 넣었다.
**여기서 안 나온다고 「자료가 없다」고 답하는 것**이 가장 나쁜 실패라서다.

### 자유 칸에는 표준이 있다 — 지어 적으면 셈이 틀어진다 (2026-08-30 실측)

직접 넣어 보고 알았다. 「사용 툴」과 「제품군」은 **정해진 값이 아니라 자유 칸**인데
뒤에 표준이 있다(도구는 기술정보 모듈의 684개, 제품군은 로드맵 정보). 서버는 안 막는다
— 막으면 사내 도구를 못 적는다. 그래서 내가 `HyperMesh`·`Icepak`·`Galaxy S`·`Fold`
라고 지어 적었고 **넷 다 표준 밖**이었다(정본은 `Altair HyperMesh`·`Ansys Icepak`·
`S 시리즈`·`A 시리즈`). 같은 것을 두 글자로 부르면 어긋남 셈이 틀어진다.

막는 것이 아니라 **보고 고르고, 뒤에 점검하는** 길을 뒀다 —
`maturity_name_catalog` 로 고르고, 넣은 뒤 `maturity_name_audit` 로 훑고,
후보가 맞으면 `maturity_rename` 으로 맞춘다. 화면에는 원래 있던 흐름이다(찾기·정돈).

### 판은 **간추려서** 준다 (2026-08-30 실측)

직접 몰아 보고 알았다 — 통짜 판은 **연계당 4,055자**다. 실제 사업부(연계 100개)면
40만 자가 넘어 AI 가 통째로 읽을 수 없다. 화면은 그걸 다 쓰지만(근거·이력·도구·부서까지
그리므로) AI 는 「지금 어느 칸인가」만 있으면 되고, 자세한 것은 그 연계 하나를 열면 된다.

`maturity_board` 는 기본이 간추린 판이다 — **86% 작다**(연계당 586자). 빠지는 것은
연계마다 되풀이되는 subject·agent 통짜와 정확도 문턱, 평가의 근거 글·근거 자료·누가
언제, 축의 묻는 말과 칸 설명. 각각 `maturity_get_pair` 와 `maturity_describe` 가 준다.
통짜가 정말 필요하면 `full=True`.

⚠️ 줄이는 것은 **이 도구의 응답**이지 백엔드가 아니다(얇은 프록시 규칙은 그대로).

### 성숙도는 절반이 척도, 절반이 **건 기록**이다

척도는 상태(연계마다 하나, 갱신됨)이고 기록은 사건(건마다 한 줄, 쌓임)이다. 둘은 서로
다른 것을 센다 — 기록을 척도처럼 고치려 하면 둘 다 뜻을 잃는다. 두 기록은 모양이
다르지만 다루는 결이 같아 `kind` 로 가른 도구 하나씩으로 묶었다(도구를 더 늘리는 것
자체가 비용이라).

**한 바퀴가 돈다** — 되풀이되는 스팟 해석을 기록으로 쌓고 → 연간 셈이 정착 후보를 내고
→ `maturity_promote_review` 로 상시 시험 항목이 되고 → 그때부터 `maturity_assess` 로
척도가 붙는다. live 시험 R 마당이 이 한 바퀴를 실제로 돈다.

### 매기기는 어디까지 되나

축의 kind 마다 손잡이가 다르다 — `maturity_describe` 가 알려 준다.

| 축 | 어떻게 |
|---|---|
| rung (연결·품질·적용 범위…) | `assess(rung=…)` |
| value (정확도·신뢰성) | `assess(value=…)` — **칸은 서버가 정한다** |
| set (자동화·시험 대체·기본 계측) | `assess(flags=[…])` |
| matrix (모델링 수준) | 바탕은 `assess(flags=…)`, 불량 유형 표는 `maturity_set_defect` |
| (모든 축) 도달 시점 | `maturity_reached` — 값 축은 안 된다 |

**일부러 안 붙인 것 둘** — 연계 끊기(`DELETE /pairs/<id>`)와 이력 한 줄 지우기.
둘 다 되돌릴 수 없고 평가·이력이 같이 사라진다. AI 가 「정리했습니다」로 지울 만한
자리라 화면에서 사람이 한다.

자료가 섞이는 일은 구조적으로 없다 — 접두사가 갈려 성숙도 도구로 대시보드를 부를 수
없고, 그 반대도 같다. 회귀시험이 두 겹으로 못 박는다(경로 검사 · live 의 K 마당).
...
| `dt3_test_maturity_mcp.py` | 38항목. 성숙도 도구 12개를 하나씩 불러 **그 경로가 백엔드 URL 표에 그 메서드로 있는지** 대본다(서버 없이 돈다) · 이름이 전부 `maturity_` 인가 · **남의 접두사로 새지 않는가** · 근거 없는 평가가 서버 전에 막히는가 |
| `dt3_test_maturity_mcp_live.py` | 76항목. 기록 한 바퀴·사전 쓰기·권한(일반 사용자)까지. 부문 셋(시뮬레이션·모니터링·디지털 스레드)을 실제로 세우고 매긴다. **경로만으로는 안 잡히는 것** — 보내는 몸통이 백엔드가 받는 꼴인지. `localhost:3003/mcp` 로 실제 도구를 부른다: 만들고·잇고·매기고, 근거 없으면 막히고, 값 축은 값만 받고, 낡은 기준이면 409, dry_run 은 저장 안 하고, **끝에 만든 것을 전부 지운다** |
