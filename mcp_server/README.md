...
`maturity_assess` · `maturity_set_defect` · `maturity_reached` 는 **바로 반영되지 않는다.**
확인 대기로 가고(202), 사람이 성숙도 화면의 「확인 대기」에서 근거를 읽고 승인해야 판에
오른다. **승인 도구는 일부러 없다** — 있으면 AI 가 자기 제안을 승인한다.

왜: 이 모듈이 지키는 한 가지는 「근거 없이는 매기지 않는다」인데, 그 규칙이 막는 것은
**빈 근거**이지 **지어낸 근거**가 아니다. 실제로 MCP 로 몰아 보다가 「낙하 시험 8건과
비교, 오차 5.4%」라는, 본 적 없는 시험을 근거로 적어 넣었고 판에는 92%(현상 재현)가
사실인 양 섰다.

나머지(대상·수단 만들기, 연계 잇기, 일괄 입력, 건 기록, 스레드 사전)는 **사실을 적는
것**이라 바로 들어간다 — 틀리면 고치면 된다. 자료 세우기까지 승인을 받게 하면
「엑셀 30줄 올려줘」가 승인 30번이 되어 아무도 안 쓴다.

승인하면 **그 사람이 매긴 것**이 된다(이력의 actor 는 승인한 사람). 「AI 가 제안했다」는
제안 표에 남는다 — 그게 감사 기록이다.

`maturity_pending` 으로 대기 목록을 읽어 **사용자에게 몇 건이 기다리는지 말해 주세요.**
아무도 안 보면 쌓이기만 하고 제안을 둔 뜻이 없어진다.
...
| `maturity_assess` | **한 축을 매기자고 제안** — 근거 필수. rung·value·flags |
| `maturity_pending` | 확인 대기 목록(읽기만 — 승인은 화면에서) |
...
⚠️ 이름 바꾸기(rename)는 **일부러 뺐다** — 사업부의 모든 줄을 한 번에 덮고 되돌릴 수 없다.
AI 는 「이런 게 표준 밖입니다」까지 말하고, 고치는 것은 사람이 화면에서 한다.
...
| `dt3_test_maturity_mcp_live.py` | 84항목. 제안 → 사람 승인 → 판에 오름까지. 기록 한 바퀴·사전 쓰기·권한(일반 사용자)까지. 부문 셋(시뮬레이션·모니터링·디지털 스레드)을 실제로 세우고 매긴다. **경로만으로는 안 잡히는 것** — 보내는 몸통이 백엔드가 받는 꼴인지. `localhost:3003/mcp` 로 실제 도구를 부른다: 만들고·잇고·매기고, 근거 없으면 막히고, 값 축은 값만 받고, 낡은 기준이면 409, dry_run 은 저장 안 하고, **끝에 만든 것을 전부 지운다** |
