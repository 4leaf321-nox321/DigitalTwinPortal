...
from app.modules.digital_twin_intel.models import (
    IntelChange, IntelEvidence, IntelLink, IntelNews, IntelTech, IntelDivisionStage)
...
def test_낡은_것만_모아볼_수_있다(db, client, auth, admin, mx):
    """
    낡음 표시가 있어도 **모아 보는 자리가 없으면** 하나씩 찾아다녀야 한다.
    ⚠️ 거르기도 서버가 한다 — 단계마다 기준 일수가 다르고, 그 표가 화면에 복제되면 갈린다.

    ⚠️⚠️ **낡음은 사업부 눈에서만 잰다**(2026-08-27). 줄 자체는 단계가 없어서,
       「여기 있다」고 말한 적 없는 것에 「그 말이 낡았다」고 할 수 없다.
    """
    from datetime import datetime, timedelta

    fresh = _cap(admin, '새 역량')
    old = _cap(admin, '낡은 역량')
    _at(admin, fresh, '관찰')
    _at(admin, old, '관찰')
    old.created_at = datetime.utcnow() - timedelta(days=400)
    row = IntelDivisionStage.query.filter_by(
        tech_uuid=old.uuid, division='MX').first()
    row.changed_at = datetime.utcnow() - timedelta(days=400)
    _db.session.commit()

    r = client.get(f'{BASE}/tech?division=MX&stale=1', headers=auth(admin))
    names = [t['name'] for t in (r.get_json() or {}).get('data') or []]
    assert names == ['낡은 역량'], names

    # 사업부를 안 고르면 잴 자가 없다 — 아무것도 안 걸린다.
    r2 = client.get(f'{BASE}/tech?stale=1', headers=auth(admin))
    assert (r2.get_json() or {}).get('data') == []
...
def test_단계를_옮기면_기록이_남는다(db, client, auth, admin, mx):
    """
    ⚠️⚠️ 단계를 「조직의 판단」이라며 관리자ㆍ사무국으로 좁혀 놓고 **그 판단의 기록이
       없었다.** 지금 값만 남으면 「왜 작년에 도입이었다가 보류로 내려갔지」에 답할 수
       없다 — 좁힌 의미가 절반이 된다.

    ⚠️ 기록에는 **어느 사업부 것인지**(`scope`)가 함께 남는다. 단계가 사업부 줄에만
       사니 그게 없으면 누구의 판단인지 모른다.
    """
    c = _cap(admin, 'Omniverse 역량')
    _at(admin, c, '시험')                              # 처음 적음
    _at(admin, c, '보류', reason='라이선스 비용')        # 시험 → 보류

    r = client.get(f'{BASE}/tech/{c.uuid}/changes', headers=auth(admin))
    ...
    assert rows[0]['scope'] == 'MX'
    # ⚠️ 처음 적은 것은 **어디서 온 것이 아니다** — 출발점이 비어 있는 것이 맞다.
    assert rows[1]['before_value'] is None and rows[1]['after_value'] == '시험'
...
def test_기술이_지워져도_기록은_남는다(db, client, auth, admin, mx):
    """무엇에 대한 기록이었는지 알아야 하므로 이름도 함께 남긴다."""
    c = _cap(admin, '곧 지울 역량')
    _at(admin, c, '시험')
    _at(admin, c, '도입')
    client.delete(f'{BASE}/tech/{c.uuid}', headers=auth(admin))

    rows = IntelChange.query.filter_by(subject_uuid=c.uuid).all()
    assert all(r.subject_name == '곧 지울 역량' for r in rows)
    fields = sorted(r.field for r in rows)
    # ⚠️ **지운 것도 기록이다.** 「이 기술이 왜 없어졌지」에 답이 없으면, 판단을
    #    좁혀 둔 뜻이 절반이다 — 지우기는 사무국만 할 수 있는 판단이다.
    assert fields == ['delete', 'stage', 'stage'], fields
    moved = [r for r in rows if r.field == 'stage']
    assert ('시험', '도입') in [(r.before_value, r.after_value) for r in moved]
