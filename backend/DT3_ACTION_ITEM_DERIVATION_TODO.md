...
> **2026-08-24 정정.** 원래 「운영 미반영. 나중에 올릴 때 이 문서대로 한다」였다.
> **2026-08-07 에 운영 반영을 마쳤다.**
>
> ⚠️⚠️ 그때 **방향을 뒤집었다.** 운영 보고가 이미 상위 액션아이템 완료일 기준으로
> 나가 있어서, 서버 규칙(세부항목 → 상위)대로 돌리면 보고한 날짜를 덮는다.
> 그래서 `dt3_align_subitems_to_action.py` 로 **상위를 정본 삼아 세부를 맞췄다.**
> **`dt3_backfill_action_items.py --commit` 을 그냥 돌리지 말 것** — 절차는
> `DT3_반출_체크리스트.md` §6-3 에 있다.
