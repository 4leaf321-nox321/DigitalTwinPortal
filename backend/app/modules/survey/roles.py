...
⚠️ **운영에는 이 값들이 쓸 만큼 채워져 있다**(2026-08-16 확인). 개발 DB 는
   아니다 — pl_knox_id 가 과제 410건 중 102건이고 계정 권한은 user·admin 뿐이라,
   **개발에서 역할이 안 잡히는 것은 데이터 탓이지 이 코드 탓이 아니다.**
   그래도 유도가 안 되는 사람은 남으므로(knoxId 미기입 과제의 PL 등),
   그 경우 응답자가 직접 고르고,
