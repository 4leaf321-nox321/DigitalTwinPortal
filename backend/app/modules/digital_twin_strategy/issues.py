...
⚠️ '발견 사항(findings)'은 여기서 다시 뽑지 않는다. 그쪽은 이미 진단 화면에서
   한 번의 클릭으로 **핵심 난제**가 되는 길이 있다. 같은 사실이 두 경로로
   들어오면 이슈 목록에 중복이 쌓이고, 어느 쪽이 정본인지 갈린다.

   **딱 하나 예외가 설문이다**(C). 지표에서 나온 발견 사항은 같은 지표가 B 로도
...
# 설문에서 나온 발견 사항의 key 앞자리. survey_link 가 붙이는 것과 같은 약속이다.
...
def derive_survey_candidates(findings, promoted=None):
    ...
    ⚠️ 지표에서 나온 발견 사항은 **여기 안 온다.** 그쪽은 B 가 같은 것을 이미 낸다.
    """
