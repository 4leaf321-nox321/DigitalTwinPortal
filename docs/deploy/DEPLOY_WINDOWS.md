...
main 푸시
   ...
   ├─ build            빌드
   └─ tag              ★ 위 둘이 통과해야 돈다
v0.1.19 태그 자동 생성  (version 이 바뀐 경우에만)
...
### ⚠️ 시험이 빨가면 릴리스가 안 나갑니다 (2026-08-25 부터)

예전에는 태그 만들기가 **별도 워크플로**(`auto-tag.yml`)라 CI 와 나란히 돌았고,
서로를 안 봐서 **시험이 빨간 채로 릴리스가 나갔습니다.** v0.7.2 와 v0.8.1 이
그렇게 나갔고 **둘 다 나간 뒤에 알았습니다.**

지금은 태그가 `CI - Windows` 안의 잡이라 `backend-tests` 와 `build` 가 통과해야
돕니다. 시험이 빨가면 태그가 안 생기고, 따라서 릴리스도 안 나갑니다.

### 🚨 급할 때 — 태그를 손으로 밀면 CI 와 무관하게 나갑니다

`Release - Windows package` 는 **태그 푸시**로 깨어납니다. 그래서 CI 가 막혔거나
급한 사정이 있으면 손으로 밀어서 내보낼 수 있습니다.
...
⚠️ **이 길은 일부러 열어 둔 비상구입니다.** 막힌 문만 있고 부술 문이 없으면
반드시 누군가 다른 데를 부수기 때문에, 부술 문을 미리 만들어 둔 것입니다.
대신 **자동이 아니라 사람이 결정한 것**이 기록에 남습니다 — 시험이 빨간 채로
나간 것이 사고가 아니라 판단이었다는 뜻이 됩니다.
...
> 레코드(`NativeCommandError`)로 감쌉니다. `flask db upgrade` 는 정상 진행
> 상황(`INFO [alembic...]`)을 stderr 로 내보내므로, 종료코드가 0 인데도
> 스크립트의 `catch` 가 걸려 `Migration failed: INFO [sqlalchemy...]` 같은
> 엉뚱한 메시지가 납니다. (2026-08-15 실제로 겪음)
>
> **두 조건이 같이 있어야 터집니다** — `2>&1` 파이프 **그리고**
> `$ErrorActionPreference = 'Stop'`. 실측:
>
> | `2>&1` | `Stop` | 결과 |
> |---|---|---|
> | ✗ | ✗ | 성공 |
...
> | **✓** | **✓** | **실패로 뒤집힘** |
>
> `Stop` 은 자동화 도구·CI·일부 래퍼가 기본으로 켜둡니다. 즉 **사람이 직접 칠
> 때는 잘 안 걸리고, 스크립트로 감쌀 때 걸립니다.** 그래서 파이프를 아예 안
> 쓰는 편이 안전합니다.
>
> 판별법: 오류 메시지에 `INFO` 나 `DEBUG` 같은 **정상 로그 문구가 박혀 있으면**
> 이 경우입니다. 진짜 실패라면 거기에 SQL 오류나 파이썬 예외가 있어야 합니다.
>
