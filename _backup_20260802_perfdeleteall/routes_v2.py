...
from app.modules.digital_twin_dashboard.assemble import assemble_data, PROJECT_COL_TO_KEY
from app.modules.digital_twin_dashboard.field_maps import (
    PROJECT_FIELD_MAP, PERFORMANCE_FIELD_MAP, translate_keys, division_kpi_code,
)
...
from app.modules.digital_twin_dashboard.models_v2 import (
    ...
    Dt2Performance, Dt2ProjectPerformance, Dt2ProjectKpi,
)
...
def list_projects():
    ...
    if request.args.get('division_id'):
        q = q.filter(Dt2Project.division_id == request.args.get('division_id', type=int))
    if request.args.get('status'):
        q = q.filter(Dt2Project.status == request.args['status'])
    if request.args.get('year'):
        q = q.filter(Dt2Project.year == request.args.get('year', type=int))
    if request.args.get('owner') == 'me':
        q = q.filter(Dt2Project.owner_user_id == actor.id)
    if request.args.get('editable') == 'true' and actor.role not in P.GLOBAL_EDIT_ROLES:
    ...
    total = q.count()
    rows = q.order_by(Dt2Project.year.desc(), Dt2Project.code).limit(limit).offset(offset).all()
...
#
# ⚠️ **관리자 전용으로 시작한다.** 기능이 아니라 **관찰**을 위해서다 —
#    LLM 이 확인 절차를 지키는지, 엉뚱한 과제를 집지 않는지는 실제로 돌려봐야 안다.
#    그때까지는 무엇이 잘못돼도 되돌릴 수 있는 사람만 쓴다.
#    (권한을 넓힐 준비가 되면 이 검사만 완화한다. 도구 쪽 권한은 이미 사용자별로 걸린다.)
#
# 도구 실행은 **호출자의 토큰으로** 돈다 — 관리자가 쓰면 관리자 권한이다.
# 즉 이 관문은 "누가 AI 를 쓸 수 있나" 이고, "AI 가 무엇을 할 수 있나" 는 그 사람 권한이다.
# ─────────────────────────────────────────────────────────────────────────────
...
def ai_agent():
    ...
    응답
        200  {answer, trace[], hops, toolCalls, model, truncated}
        400  query 없음
        403  관리자가 아님
    ...
    if actor.role != UserRole.ADMIN:
        return error_response(
            'AI 에이전트는 현재 관리자만 사용할 수 있습니다.', status_code=403)
...
def patch_project(uuid):
    ...
    # (c-2) AI 가 건드리면 안 되는 필드 — 확인 대기로도 안 보낸다. 바로 거절한다.
    #
    # 참여인력·소유자는 **이름만 보고 예/아니오 할 수 없다**(동명이인). 그리고 여기
    # 이름이 들어가면 그 사람에게 이 과제 편집 권한이 생긴다 — 되돌려도 그 사이는 열려 있다.
    # 그래서 확인 절차로 감싸지 않고 전용 화면으로 보낸다. 자세한 이유는 permissions 참고.
    if is_ai:
        ...
        if forbidden:
            ...
            return error_response(
                ...
                '참여인력을 넣으려면 `과제참여인력목록` 을 knoxId 와 함께 쓰세요.',
    ...
    if is_ai and cls.core:
        before_values = {k: _readable(getattr(p, k, None)) for k in cls.core}
        ...
        # 저위험 필드가 섞여 있으면 그건 즉시 반영한다
        applied = _apply(p, cls.low_risk, caller, subject, delegated,
                         source='ai', reason=body.get('reason')) if cls.low_risk else []
        db.session.commit()
        return success_response({
            'applied': applied,
            'ignored': ignored,
            'proposalId': proposal.id,
            'pendingFields': sorted(cls.core),
    ...
    db.session.commit()
    return success_response({'applied': applied, 'ignored': ignored,
...
def create_project():
    ...
    return success_response(detail, message='과제가 생성되었습니다.', status_code=201)
...
def _proposal_dict(pr: Dt2ChangeProposal, project=None, names=None):
    ...
    return {
        ...
        'currentVersion': project.row_version if project is not None else None,
        'reason': pr.reason,
...
def list_proposals():
    ...
    projects = {}
    for pr in rows:
        if pr.project_uuid not in projects:
    ...
    for pr in rows:
        ...
        if not allowed and not (status != 'pending' and P.can_edit_project(actor, proj)):
            continue
...
def create_performance():
    ...
    for k, v in {**cls.low_risk, **cls.core}.items():
        setattr(p, k, v)
    db.session.add(p)
    db.session.flush()
    ...
    detail = _perf_dict(p)
    detail['ignored'] = ignored
    return success_response(detail, message='성과가 생성되었습니다.', status_code=201)
...
def patch_performance(uuid):
    ...
    if is_ai and cls.core:
        db.session.rollback()
        return error_response(
            'AI 는 성과의 핵심 필드를 직접 고칠 수 없습니다. '
            '이 성과를 쓰는 다른 과제의 숫자까지 바뀌기 때문입니다.',
            errors=sorted(cls.core), status_code=403)
...
def replace_project_kpi_links(uuid):
    ...
    body = request.get_json(silent=True) or {}
    if body.get('actor_mode') == 'ai':
        # 성과 연결과 달리 남의 과제 숫자를 흔들지는 않는다. 그래도 지금은 막아 둔다 —
        # 이 연결은 사람이 "이 과제가 무엇에 기여하는가" 를 선언하는 값이고,
        # AI 가 추측으로 채우면 매트릭스의 빈칸(=계획의 구멍)이 가짜로 메워진다.
        # 열려면 제안 큐(dt2_change_proposals)를 거치게 하는 것이 맞다.
    ...
    if not all(isinstance(it, dict) for it in items):
        return error_response('items 원소는 모두 객체여야 합니다.', status_code=400)
    ...
    # id 를 정수로 확정한다. 문자열이 와도 받아주되, 숫자가 아니면 400 이다
    # (그대로 흘리면 FK 위반이 500 으로 나간다).
    wanted = []
    for it in items:
        ...
        try:
            ...
            db.session.rollback()
            return error_response(f'kpiDefinitionId 는 정수여야 합니다: {raw!r}',
                                  status_code=400)
    ...
    for it, kid in zip(items, wanted):
        raw = it.get('targetDivision')
        target = str(raw).strip() if raw not in (None, '') else ''

        if not is_functional:
            # 사업부 과제는 **자기 사업부 고정**이다 (2026-08-01 결정).
            # 보낸 값이 달라도 조용히 고치지 않는다 — 화면과 서버가 다른 규칙을
            # 갖고 있다는 뜻이라, 그대로 알려주는 편이 낫다.
            if target and target != (p.division or ''):
                db.session.rollback()
                return error_response(
                    f'{p.division} 과제는 자기 사업부의 지표만 연결할 수 있습니다 '
                    f'(보낸 대상: {target}).', status_code=400)
            target = p.division or ''
            if not target:
                db.session.rollback()
                return error_response(
                    '과제에 사업부가 지정되어 있지 않아 KPI 를 연결할 수 없습니다. '
                    '기본정보에서 사업부를 먼저 지정하세요.', status_code=400)
        else:
            # 기능조직은 **반드시 지목**해야 한다. 자기 사업부엔 지표가 없다.
            if not target:
                db.session.rollback()
                return error_response(
                    f'{p.division} 은(는) 기능조직이라 지원할 사업부를 골라야 합니다.',
                    status_code=400)

        if target not in owners:
            db.session.rollback()
            return error_response(
                f'{target} 은(는) DX KPI 를 관리하는 사업부가 아닙니다.',
                status_code=400)

        # 사업부 전용 지표를 엉뚱한 사업부 대상으로 걸 수 없다.
        # (예: 물성 DB 확보 진척률[vd] 을 MX 대상으로) — 화면도 막지만 서버가 정본이다.
        scope = found.get(kid) or []
        if scope and owners[target]['code'] not in scope:
            db.session.rollback()
            return error_response(
                f'이 지표는 {target} 에서 관리하지 않습니다 '
                f'(대상: {", ".join(scope)}).', status_code=400)

        resolved.append((kid, target))

    if len(set(resolved)) != len(resolved):
        db.session.rollback()
        return error_response('같은 KPI 를 같은 대상 사업부에 두 번 연결할 수 없습니다.',
                              status_code=400)

    before = [_kpi_link_dict(ln, kdef) for ln, kdef in _kpi_links_of(uuid)]

    Dt2ProjectKpi.query.filter_by(project_uuid=uuid).delete()
    db.session.flush()
    for it, (kid, target) in zip(items, resolved):
        note = it.get('note')
    ...
    after = [_kpi_link_dict(ln, kdef) for ln, kdef in _kpi_links_of(uuid)]
    if before != after:
        _log_link_change(p, before, after, actor, 'ui', body.get('reason'),
                         field='kpi_links')
    db.session.commit()
...
def _bulk_delete_actor():
    """
    연도별 일괄 삭제 구역의 주체. `(actor, None)` 또는 `(None, 오류응답)`.

    권한 판단을 이 한 함수에 모은다 — 조회(연도 요약)와 실행이 **같은 조건**이어야
    화면에 목록이 보이는데 누르면 403 같은 어긋남이 안 생긴다.
    """
    # PAT 은 `_actor()` 안에서 먼저 처리되므로 그 전에 걸러야 한다.
    if pat_user() is not None:
        return None, error_response(
            '연도별 일괄 삭제는 외부 토큰(PAT·MCP)으로는 호출할 수 없습니다. '
    ...
    if not (actor.role == UserRole.ADMIN or actor.is_admin):
        return None, error_response(
            '연도별 일괄 삭제는 관리자(admin)만 할 수 있습니다.', status_code=403)
    return actor, None
