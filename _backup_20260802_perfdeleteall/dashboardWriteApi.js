...
const createProjectFlowV2 = async ({ project, performances, activityLogs }) => {
  ...
  // 성과 연결은 **과제가 있어야** 걸 수 있으므로 순서를 뒤집을 수 없다.
 ...
 *
 * 성과 연결이 딸려 있으면 생성 **뒤에** 연결 API 를 부른다 — 수정과 달리 순서를
 * 뒤집을 수 없다(과제가 있어야 연결을 건다). 선행과제·이미지가 딸린 경우는 아직
 * 보낼 길이 없어 V1 로 보낸다.
 */
 ...
 *
 * 성과 연결은 연결 API 로, 이미지는 `image_refs_json` 으로 보낼 수 있게 됐다.
 * 남은 건 선행과제뿐이다.
 */
const RELATION_FIELDS_WITHOUT_API = ['선행과제목록'];
...
const resolvePerformanceUuid = (ref, performances) => {
  ...
  const hit = list.find(p => p?.uuid === ref
...
const toLinkItems = (list, performances) => normalizeLinks(list).map(link => ({
  performanceUuid: resolvePerformanceUuid(link.ref, performances),
  contribution: link.기여도 === '' ? null : link.기여도,
  actualLevel: link.실적 === '' ? null : link.실적,
}));
...
const patchProjectEditV2 = async ({ project, before, performances, activityLogs }) => {
  ...
  if (Object.keys(patch).length === 0) {
    await sendActivityLogs(activityLogs);
    return { applied: [], ignored: [], links: linkResult };
  }
  ...
  await sendActivityLogs(activityLogs);
  return { ...result, links: linkResult };
};
 ...
 *
 * 성과 연결이 바뀌면 **연결 API 를 먼저** 부르고 그다음 필드를 PATCH 한다(2026-07-30).
 * 아직 API 가 없는 관계 키(선행과제·이미지)가 바뀐 저장만 V1 로 물러선다.
 */
const saveProjectEditCore = ({ projects, performances, metadata, activityLogs,
  ...
  // 선행과제·이미지는 아직 보낼 길이 없다. 그냥 보내면 그 변경만 조용히 사라지므로
  // 저장 전체를 V1 로 보낸다. (성과 연결은 이제 API 가 있어 여기 해당하지 않는다)
  ...
  return patchProjectEditV2({ project, before, performances, activityLogs }).catch(err => {
